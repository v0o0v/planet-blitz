...
| `ship_striker.glb` | 플레이어 기체(typeId 0) — 청록 인터셉터 | 2,975 | 302 KB |
...
### ⚠️ 함정 ④ — 생성물의 **기수 방향**은 지정할 수 없다

프롬프트에 "front / nose cone" 을 적어도 Meshy 가 그 축을 어디로 뽑을지는 통제되지 않는다. 스트라이커는
**기수가 −X** 로 나왔고, 이 리포의 기체 규약은 **+X**(`src/render/textures.ts`)다. 모델을 다시 뽑는 대신
`shipActor.ts` 의 `SHIP_MODELS[].orient`(정규화 그룹 **안쪽**에 걸리는 Euler)로 한 번 돌린다 — 20 크레딧을
아끼고, 자세 보정이 연출 회전과 섞이지 않는다. **새 기체를 뽑을 때마다 뷰어에서 확인할 것.**
...
`/ship3d.html` — 플레이어 기체 뷰어. 칸마다 조종 상황(정지·순항·선회·가감속·대시·피격)을 **대본으로
재생**한다. `?t=<초>` 를 붙이면 rAF 대신 **고정 스텝**으로 그 시각의 포즈를 한 번만 그린다 —
헤드리스 Chrome 은 rAF 를 돌리지 않아 그냥 찍으면 화면이 검게 나온다.

인게임 확인은 하네스(`?harness=1`) → 보스전 탭.

기체 연출은 보스와 구조가 다르다: 보스는 상태가 열거형(phase 0/1/2)이라 모션 **함수**를 갈랐지만,
기체는 선회율·속도·가속이 연속이라 **축마다 하나의 전달함수**를 둔다(롤=선회 · 피치=가감속+호흡 ·
발광/신축=대시 · 지터=피격). ⚠️ **요(yaw)는 3D 에 없다** — 기수 방향은 Pixi 스프라이트 회전(`shipFacing`)이
쥔다. 3D 에서 또 돌리면 이중 회전이라 조준선과 기수가 어긋난다.
