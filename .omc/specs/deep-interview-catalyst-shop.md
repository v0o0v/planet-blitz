...
- [ ] `가격 = 10 × (10 / dropWeight)` 순수 함수가 48종 전부에 대해 **정수로 절하한** 값을 반환한다
      (개정 2026-07-31 — `W_SIGNATURE = 8` 이라 특산 12종이 `12.5` 로 비정수다. `Math.floor` 로
      절하해 12 를 쓴다. 양수 구간에서 JS 와 PostgreSQL 의 `floor` 가 동일해 표시가와 청구액이
      갈릴 여지가 없다. 원문은 *"정수를 반환한다"* 였고 이는 카탈로그와 맞지 않았다)
