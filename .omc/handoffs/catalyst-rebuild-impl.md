...
9. ⛔ **보상 축 분포가 헌장 쿼터를 벗어났다 — 미결(사용자 판정 필요).**
   위 6·8 이 둘 다 자원→드랍이라 실측이 **드랍 17 · 자원 9**(희귀도 10 · XP 8 · 촉매 4)가 됐다.
   자원 하드 상한 ≤12 는 충족하지만 **드랍이 "12 안팎"을 5 초과**한다. 다른 카드 축을 임의로
   옮겨 맞추지 않고 그대로 뒀고 `tests/catalysts.test.ts` 가 17/9 를 잠근다.
