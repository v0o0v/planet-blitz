...
## ✅ 커밋 2 는 완료됐다 (2026-08-06) — 아래 표는 이력으로 남긴다

11항목 전부 + 스트라이커 시그니처(합침) + 골든 3종 재생성까지 끝났다. **다음 레인은 아래
「커밋 2 이후」부터 읽어라.**

### 구현하며 드러난 것 — 다음 레인이 알아야 할 5건

1. **`tsc` 는 위치 인자 삭제를 전부 잡지 못한다.** `computeLoadoutStats` 의 가운데 인자를
   지웠는데 `(x, undefined, 5000)` 이 신 시그니처에서 `typeId = 5000` 으로 **말이 되어**
   통과했다(범위 밖은 스트라이커로 정규화 → 계보 보너스 소멸). 전체 스위트가 잡았다.
   **위치 인자를 지울 때는 "타입이 우연히 맞는" 조합을 먼저 세어라.**
2. **「시그니처 없음」이 제품에서 사라졌다.** 7기체 전부 비트를 가지므로 어떤 유효 typeId 로도
   시그니처 off 가 안 나온다. 대조군은 **트리거 굶기기**(같은 기체·같은 마스크로 두고 트리거
   aux 만 매 틱 되돌린다)로 재설계했다 — 기체를 갈아치우던 구 방식보다 오염이 적다.
3. **길이 프리픽스 폴드 커버리지가 구조적으로 사라졌다.** 해츨링 78칸이 유일한 근거였는데
   전 기체 30 통일로 길이가 다른 유효 config 가 없다. `shipSkillLayout` 의 단위 단언으로 이관.
4. **`replay.ts` 주석이 거짓이었다** — "길이 프리픽스로 미존재/빈 벡터를 구분" 은 사실이 아니다
   (`undefined` 와 `[]` 가 둘 다 0 을 접는다). 그리고 그게 옳다. 주석을 고치고 두 사실을
   단언으로 못 박았다.
5. **골든 재생성은 검증이 아니다.** 다시 뜬 골든으로 그 골든을 검사하면 항진이다. 근거는
   재생성 **전에** 통과시킨 불변식이어야 한다 — 다음 회차(스킬 배선)도 같은 순서를 지켜라.

## (이력) 커밋 2 가 한 원자로 묶은 것
