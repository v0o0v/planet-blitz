...
6. **유니크** — 15점 중 5점(고유 효과가 시뮬에 훅. 확정 목록: §8 OQ-M2-6).
...
- [ ] **AC1 (아이템 모델)**: `rollItem(dropSeed, rarity, source)`가 동일 입력에 동일 아이템(등급·어픽스 목록·수치)을 반환 — 순수성 단위 테스트. 매직 어픽스 1~2, 레어 3~6 개수 규칙 준수(M2 어픽스 풀 21종 기준, 원소 3종 제외).
...
- **A2. 어픽스 풀 데이터** — 신규 `data/affixes.ts`
  ...
  - **M2 시작 풀은 21종** — 접두 중 원소 3종(화염·냉기·전격)은 상태이상 시스템과 함께 M3에서 투입(OQ-M3-5 결정, `deep-interview-m2-m5-open-questions.md`). M2에서는 접두 9 + 접미 12 = 21종만 데이터화, 나머지 3종은 M3 Phase B4에서 추가.
...
- **F1. 유니크 5점** — `data/uniques.ts` + `src/items/loadout.ts` 훅
  - 15점 중 5점 확정 구현(배분 원칙: 주무기 3타입 대표 각 1 + 보조무기 1 + 범용 1 — §8 OQ-M2-6 확정 표): ①발칸 "과열 드럼"(연속 명중마다 발사속도 스택↑, 피격 시 리셋) ②스프레드 "분열 코어"(스프레드 탄 명중 시 파편 2발 분열) ③레일건 "관통 자이로"(관통 무제한+관통당 피해 증폭) ④보조무기 "자율 드론 베이"(발동 시 임시 포탑 소환, `turretPickup` 로직 재사용) ⑤범용·장갑 "위상 장갑"(대시 직후 짧은 무적+대시 쿨다운 감소). 이름은 구현 시 조정 가능, 효과 골자는 고정. 고유 효과가 `computeLoadoutStats`에서 시뮬 파라미터로 훅(무기 타입 변형·특수 거동). 나머지 10점은 M3.
...
- **OQ-M2-6** `[확정]`: 유니크 5점 선정 목록 — deep-interview로 확정(스펙: `deep-interview-m2-m5-open-questions.md`). 배분 원칙: **주무기 3타입 대표 각 1 + 보조무기 1 + 범용 1**.
  ...
  이름은 구현 시 조정 가능, 효과 골자는 고정. **어픽스 풀도 함께 확정**: 원소 3종(화염·냉기·전격)은 M2 제외, M3에서 상태이상 시스템과 함께 투입 — M2 어픽스 풀은 21종으로 시작(§A2).
