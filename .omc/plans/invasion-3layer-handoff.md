...
> ### ⚠️ 다음 세션 착수 시 **가장 먼저** 할 것
> 배포는 **세션 초반 컨텍스트가 넉넉할 때** 실행하라. MCP `apply_migration`/`deploy_edge_function` 은 SQL·번들을 **인라인 인자로** 받으므로, 파일을 읽고 다시 넘기는 왕복에 **약 110k 토큰**이 든다(마이그레이션 5종 ~115KB + EF 번들 ~126KB). 세션 후반에 시작하면 배포 시퀀스 도중 컨텍스트가 압축되고, **마이그레이션 5종 중 일부만 적용된 중간 상태**가 가장 나쁜 결과다.
>
> 대안 경로가 없음을 확인했다: `supabase` CLI 2.101.0 은 설치돼 있으나 `config.toml` 도 링크도 액세스 토큰도 없어 인증 불가. `psql` 없음. `.env` 에 service role key 없음. **MCP 가 유일한 경로다.**
>
> 서브에이전트에 위임하면 컨텍스트를 아낄 수 있으나, 이전 세션에서 원격 DB 마이그레이션을 지시하는 Agent spawn 이 **분류기에 차단**된 전례가 있다. 차단되면 리드가 직접 수행할 것.
>
> **지연이 안전한 이유**(실측): 원격 `invasions=0` · `invasion_snapshots=0` · 미출시 · 유일한 실유저 방어는 래더 미배치(`placed=null`)라 침공 대상이 될 수 없다. 즉 배포 전까지 깨질 사용자 경험이 없다.
