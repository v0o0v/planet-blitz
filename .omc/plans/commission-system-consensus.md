...
- 결정 문서: ADR-0046(구 ADR-0042 — 촉매 상점 ADR 과 번호 충돌로 리넘버) · ADR-0043 · ADR-0044 · **ADR-0045**(신규 — 의뢰 확정 지급물 서버 권위)
...
- **Option C (구간마다 별개 런) — 기각.** ADR-0046 이 이미 기각("하나의 여정" 체감 소멸 + 의뢰 진행 상태를 서버 상태 기계로 관리해야 하고 이탈·재접속이 새 공격면).
...
- `SegmentSpec` 에 **`mode` 필드를 두지 않는다.** `planetMode` 는 `planetContent(planet).mode` 파생이 단일 정본이다(`src/run/runConfig.ts:186`).
  → **명시적 비목표**: 행성-모드 자유 조합("카르곤을 수축 모드로")은 범위 밖. ADR-0046 의 "구간마다 모드를 갈아탄다"는 실질적으로 "구간마다 행성을 갈아탄다"이며, 이 등가를 ADR 에 명시한다.
...
ADR-0046 결과 절이 "난이도 기준선을 조합 단위로 재야 한다 — 모드별 단일 계측에서 자동으로 파생되지 않는다"를 명시했는데 rev4 에 계측 단계가 없었다.
