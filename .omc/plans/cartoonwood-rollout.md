...
| 6 | 관제탑 (ControlTower) | `src/ui/pixi/controlTower.ts` | **완료** (2026-07-20) | 앞선 다섯 화면과 골격이 다르다 — 세로 스택이 아니라 **열 보드**다. 위에 상태 패널(결과 배너·검증 중·배치전 진행바·상대 카드 정찰 공개 — 내용 높이를 재서 자란다), 아래에 대상 목록 / 기지 정찰 / (소식) / 순위표. **소식(복수전·알림)이 있을 때만 4열**이 되고 없으면 3열이 넓어진다(성계 지도 변칙 패널과 같은 규칙). 목록 셋은 전부 마스크 스크롤 + 행 경계 클램프. `hexColor` 를 `theme.ts` 로 승격(성계 지도와 공유 — 소비자 2곳). |
| 7 | 카드 상점 (CardsView) | `src/ui/cardsView.ts` | **다음** | 서버 권위 구매 경로 주의(CORS/거부 코드 매핑 기존 작업 참조). |
