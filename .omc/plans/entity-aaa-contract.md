...
카르곤 보스전 실측(시드 1, 틱 885, sim 해시 동일 `cfd82d70`):

| 지표 | 통합 전 | 통합 후(1차) | 배수 |
|---|---|---|---|
| bright(L≥96) 면적 | 3.49% | **14.55%** | ×4.2 |
| p95 명도 | 85.19 | **206.89** | ×2.4 |
| mean 명도 | 36.17 | 54.70 | ×1.5 |

증상은 미관이 아니라 **게임플레이 파탄**이었다: 보스전 중앙이 흰색으로 포화돼 플레이어 기체가
삼켜졌다. 기준선의 결함이 "플레이어를 찾는 데 시간이 걸린다"였고 레인 A 의 목적이 정확히 그
해결이었는데, 통합 결과는 **의도와 정반대**였다.
