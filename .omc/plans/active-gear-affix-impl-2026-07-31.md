...
> 📌 **2026-07-31 후속 조사 완료 — 정본은
> [`.omc/research/existing-defects-active-power-loadout-clamp-2026-07-31.md`](../research/existing-defects-active-power-loadout-clamp-2026-07-31.md).**
> 아래 A-1·A-2 는 그 조사가 **부분 정정**했다. 요지:
> - A-1: 호출은 **17종**(`bruiser.ts:94` 누락). 완전 반응 14 / **부분 반응 3**(만료 훅 피해만) /
>   무반응 **25**. 그 3종은 조건을 안 세우면 관측에서 "불변"으로 보여 **음성 케이스가 거짓**이 된다.
>   검증 축 항진은 뮤테이션으로 확증(소비 절단 시 5,334 중 `denoFixture` 1건만 실패, AC-13 전원 통과).
>   권고는 **(가) 범위 축소·명문화**.
> - A-2: **잘못된 위치의 클램프는 0건**이고 기존 상한 3종은 전부 소비 지점에 있다. 실체는 위치가
>   아니라 **부재**이며, ADR-0028 이 파워 상한을 배제했으므로 방어 가능한 것은 **DoS 축**뿐이다
>   (`bulletCountAdd` 정직 상한 13 vs 위조 2,000 → 재실행 ≈89초로 EF 사망).
