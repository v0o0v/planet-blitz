...
### sim 배선 현황 — **6종 전량 실동작** (2026-07-21 갱신)
`src/sim/world.ts` 의 `stepShipSignature` + 소비 지점이 **시그니처 6종 전부**를 게이트한다. 아래 "신규 2종 우선 출시" 절은 **역사적 기록**이며 현재 사실이 아니다(그 시점의 판단 근거로만 읽을 것).

| 타입 | 비트 | sim 배선 위치 | aux 슬롯 |
|---|---|---|---|
| 1 `bruiser` | 18 | `stepShipSignature`(스택 소멸 타이머) · `resolveCollisions`(감소 + 적립) | aux0 = 스택(0..8) · aux1 = 마지막 피격 이후 틱 |
| 2 `arccaster` | 19 | `stepShipSignature`(정지 카운터) · `autoAttack`(피해 증폭) | aux0 = 연속 정지 틱 · aux1 미사용 |
| 3 `phantom` | 20 | `stepShipSignature`(무피격 사이클) · `src/sim/cloak.ts` `playerCloaked` ← `patterns/index.ts` 3곳 + `boss.ts` 1곳 · `autoAttack`(해제 첫 타 배율) | aux0 = 무피격 틱(0..359) · aux1 = 배율 토큰(0/1) |
| 4 `hatchling` | 21 | `stepShipSignature` → `stepHatchBrood`(병아리 출격) | aux0 = 마지막 출격 시점 `state.kills` 스냅샷 · aux1 미사용 |
| 5 `mallow` | 22 | `stepShipSignature`(정산) · `resolveCollisions`(지연 전환·적립) | aux0 = 지연 피해 풀 · aux1 = 연속 무피격 틱 |
| 6 `bubble` | 23 | `stepShipSignature`(재생 타이머) · `resolveCollisions`(흡수) · `burstFilm`(파열 밀어내기) | aux0 = 막 내구(0..60) · aux1 = 파열 이후 틱 |

**스트라이커(0)는 시그니처를 의도적으로 보유하지 않는다**(§11) — `signatureBit === -1` 이라 `computeActiveSignature` 가 -1 을 돌려주고, 신규 코드가 한 줄도 실행되지 않아 W0 골든 33건과 Deno 픽스처 ①~⑥ 이 바이트 불변이다.

#### 런당 시그니처는 **정확히 하나**로 정규화된다
`state.sigBit`(= `computeActiveSignature(config)`, `createWorld` 가 1회 계산)이 정본이고 모든 게이트가 그 값과 `===` 비교한다. 마스크에 시그니처 비트가 둘 이상 켜지면 **최저 비트 하나**만 살아남는다. 예전의 비트별 독립 OR 판정은 `stepShipSignature`(첫 분기 return)와 소비 지점(비트마다 독립 판정)이 어긋나 **aux 슬롯이 별칭**이 됐다 — 마스크 18｜20 런에서 팬텀 소비자가 브루저의 장갑 소멸 타이머를 배율 토큰으로 읽어 거의 모든 발사에 2.5배가 실렸다(실측 kills 41→45). 침공 EF 는 공격자 loadout 을 검증 없이 재실행에 쓰므로 이 위조는 내적 일관 상태로 accept 됐다.

#### 팬텀 은신 규칙 (설계서가 비워 둔 부분을 배선이 정한 값)
연속 무피격 `CLOAK_UNHIT_TICKS`(240)를 채우면 `CLOAK_HOLD_TICKS`(120) 동안 은신하고, 그 뒤 자동 해제돼 다시 240틱을 채워야 한다. **피격은 사이클을 즉시 0 으로 되돌린다.** 배율 토큰은 진입 틱에 1회만 서고 주무기 발사로 소진된다(발사가 은신을 풀지는 않는다).
 · 초판 "발사하면 풀린다" 는 자동 조준 sim 과 양립하지 않아 은신 점유율이 **0.00~0.10%** 였다(사실상 미배선).
 · 그 반대인 "피격으로만 풀린다" 는 원거리 견제형만 있는 무대에서 **영구 은신**(3600틱 중 3361틱)이 됐다.
 · 현 규칙은 점유율이 구조적으로 120/360 = 1/3 이하로 유계다(실측 p0t0 5400틱 중 521틱 ≈ 9.6%).
침공(3레이어)에서는 **시그니처 전체가 비활성**이다 — 억제(대가)를 걸 수 없는데 배율(이득)만 남기면 침공에서 공짜로 강해지므로 대칭을 택했다.

### 신규 2종 우선 출시 = **브루저 + 아크 캐스터** (근거는 취향이 아니라 구현 위험 — **역사적 기록**)
