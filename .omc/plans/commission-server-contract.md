...
- 결정 문서: **ADR-0046**(의뢰 런 모드 전환 — 2026-07-31 PR#224 로 0042 에서 리넘버, 0042 는 촉매 상점) · ADR-0043 · ADR-0044 · ADR-0045(3차 정정본) · ADR-0027 · ADR-0028 · ADR-0026 · ADR-0024 · ADR-0039 · ADR-0038
- **이 문서의 모든 사실 주장은 `파일:줄` 을 달거나 "미확인"으로 표시한다.**
- **개정: rev4** (구현 보안 검토 반영 — CRITICAL 1 · MAJOR 3 · MINOR 4. §0-4 참조. rev3 의 결정을 지우지 않고 그 자리에 남긴다.)
...
### 0-4. rev4 가 rev3 에서 뒤집은 것 (구현 검토 반영, 2026-08-01)

구현을 대상으로 한 보안 검토가 **CRITICAL 1 · MAJOR 3 · MINOR 4** 를 냈다. 계약이 요구한 방어
9개는 **전부 코드에 있었고**, 결함은 전부 **계약이 못 본 자리**에서 나왔다.

| # | rev3 | rev4 | 왜 rev3 이 틀렸나 |
|---|---|---|---|
| **C1** | §7 게이트 6 = "서버 payload 로 `commission` 블록을 **덮어쓴다**. 강제는 게이트 7 재실행이 진다" | 게이트 6 = **allowlist 재조립.** 코어 스탯 9종은 `DEFAULT_CONFIG` 고정 · 무대는 payload 1구간 파생 · `planetMultCenti`/`planetMultEpoch`/`invasion3`/`pilot`/`catalysts` 는 **키째 제거** · `maxSegments` 는 서버 상수 | **"재실행이 강제한다"가 거짓이었다.** 재실행이 제출자 config 로 돌면 해시는 제출자가 **같은 config 로 계산한** claim 과 일치하므로 게이트 7 은 **원리적으로 발산하지 않는다**. 덮어쓰지 않는 필드에는 강제가 아예 없다: `playerHp:1e9`(무손실 완주 → 확정 지급물) · `planetMultCenti:1e6`(자원 무제한) · `planet`/`stage`(**1구간 무대는 서버 권위가 아니다** — `stageOverride` 는 `nextIndex>=1` 전환에만 적용되고 `createWorld` 는 `segments[0]` 을 읽지 않는다) · `maxSegments:0`(보스로 즉시 점프) |
| **C2** | §5 "의뢰는 **서버 재실행 증거가 있으므로 증거가 캡을 대체한다**" → 개연성 캡 면제 | 면제는 유지하되 **캡을 `settle_commission` 의 런 파생분으로 옮긴다** | 대체한다는 그 증거가 공격자 config 로 만들어졌다(C1). 다만 **합에 캡을 걸면 안 된다** — 확정 보상은 서버가 payload 에 구운 값이라 개연성을 물을 대상이 아니고, 합을 런 길이로 캡하면 **짧은 고계급 의뢰의 확정 지급이 조용히 잘린다** |
| **M1** | §2-2 "트리거 본문 전체를 서브트랜잭션으로" → 구현은 피호출자만 감쌈 | **트리거 함수 자체도** 감싼다 | 피호출자 안의 핸들러는 트리거 함수의 **레코드 필드 해석**(`new.summary` 등)을 보호하지 못한다. 그 컬럼은 나중에 `add column` 으로 붙은 것이라 장래 리네임의 실제 후보이고, 터지면 PR#222 와 같은 형상(전 플레이어 자원 미지급 + 무증상)이다 |
| **M2** | §4 게이트 3(로드아웃 대조) + 게이트 4(미인가 유니크)를 2겹으로 상정 | **0겹임을 인정하고 대체 방어 3겹을 명시 배정** | 게이트 3 의 기준값 `loadout_sealed` 는 클라가 넣은 값이고(출격 전 위조 열림, §2-3 과 같은 결), 게이트 4 는 `COMMISSION_EXCLUSIVE_UNIQUE_BITS` 가 비어 구조적으로 아무것도 안 본다. **자인 둘이 겹쳐 상쇄됐다** |
| **m1** | §7-1 게이트 표 순서: 0b(멱등 반환) → 0d(소유자) | **0d 를 0b 보다 앞으로** | 남의 `run_id` 를 알면 그 런의 `verified_result`(finalHash·틱수·승패·지급액)를 읽는다. uuid v4 라 착취성은 낮지만 **소유자 검사보다 앞서는 반환 경로를 두지 않는다**가 더 싼 규율이다 |
| **m2** | §3-5 컬럼 GRANT 회수를 `authenticated` 에만 | `anon` 에서도 회수 | "행은 RLS, 컬럼은 GRANT — **각각** fail-closed" 가 anon 에 대해 1겹뿐이었다 |
| **m3** | `revoke execute … from public` 을 신규 함수 8종에 | 트리거 함수 포함 **9종 전부** | 착취 경로는 없지만(Postgres 가 `returns trigger` 직접 호출을 거부) 예외가 하나 있으면 회귀 대조에서 "빠뜨린 것"과 "일부러 뺀 것"을 구분할 수 없다 |
| **m4** | §8 중복 리플레이가 "계정당 CPU 상한을 2배로" | **런당 +1회**로 정정 | `extractRunResources` 는 accept 경로 전용이고 accept 는 `run_id` 당 최대 1회다(재호출은 게이트 0b 가 CPU 없이 반환). 남는 창은 `settle_commission` 500 뒤의 재시도인데 `verify_attempts`(상한 5)가 묶는다 |

**⚠️ 그리고 C1 은 AC-E7 이 초록인 채로 전부 통과했다.** AC-E7 은 "서버 payload 와 다른 **무대**를
전제한 입력은 게이트 6 덮어쓰기 후 해시 발산으로 거부된다"인데, 이는 **게이트 6 이 덮어쓰는
필드에 대해서만** 참이다. 덮지 않는 필드에는 발산이 원리적으로 없다. rev1 의 "AC 가 6건 중
하나도 안 잡는다", rev2 의 R 계열 재발에 이어 **세 번째**다 — AC 를 쓸 때 **그 AC 가 무엇을 안
보는지**를 함께 세지 않으면 같은 실패가 계속 반복된다는 증거다.

**rev4 의 회귀 게이트**: `tests/verifyCommission.test.ts` 의 "게이트 6 — 제출 config 를 정화한다"
블록. **진단력을 실측했다** — 게이트 6 을 rev3 의 덮어쓰기로 되돌리면 4건이 빨간불이 되고,
정직한 제출 대조군은 그대로 통과한다(항진 아님).
