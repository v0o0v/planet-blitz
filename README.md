...
**독점 저작물이다 — 소스가 공개돼 있는 것은 열람 허용이지 이용 허락이 아니다.** 복제·수정·
배포·상업적 이용은 사전 서면 허가가 필요하다. 전문은 [`LICENSE`](LICENSE).

**예외: `assets/audio/**` 의 BGM 6트랙은 CC0 1.0(퍼블릭 도메인)이라 자유롭게 쓸 수 있다**
(출처·저작자는 [`assets/audio/CREDITS.md`](assets/audio/CREDITS.md)). 자산을 추가할 때
라이선스가 다른 것이 들어오면 `LICENSE` 의 제3자 자산 절에 같이 적을 것 — 안 적으면 독점
고지가 남의 자산까지 덮는 것처럼 읽힌다.
