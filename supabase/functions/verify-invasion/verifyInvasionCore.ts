 ...
 *
 * ⚠️ 잔여 신뢰 범위(문서화된 한계, ADR-0028 연기 확정): 이 코어는 **방어 배치**의 정당성만
 * 서버 DB와 대조한다. 공격자 자신의 로드아웃(config.loadout·skillInvest)이 실제 보유 장비와
 * 일치하는지는 대조하지 않는다 — 재실행은 제출된 공격자 로드아웃을 그대로 쓰므로,
 * 자기 로드아웃을 부풀린 위조는 내적으로 일관돼 accept될 수 있다. 이는 "방어를 약화해
 * 승률을 조작"하는 래더 오염(중대·이미 defense-mismatch로 차단)과 달리 공격자 자신을
 * 강화하는 축이다.
 *   ⚠️ 후속 게이트는 "공격자 ships/items 서버 대조"가 **아니다** — ships/items 는 클라 rw
 *   미러(ships_rw_own FOR ALL)라 위조 기체를 써 넣고 일치 로드아웃을 제출하면 대조를
 *   통과한다(클라가 쓸 수 있는 미러와의 대조는 방어가 아니다). 진짜 차단은 아이템 획득·
 *   XP/레벨 진행을 **서버 권위 원장**으로 옮긴 뒤에만 성립하며, 이는 다세션 규모 재설계라
 *   ADR-0028 로 연기했다(무료 티어 비용·재화 위조가 이번 우선순위, 로드아웃 위조는 래더만
 *   오염시키며 비용 무관). 미봉책(파워 절대 상한 클램프)은 정직 강자 오거부 회귀라 배제한다.
 */
