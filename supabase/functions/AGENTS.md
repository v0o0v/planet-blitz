...
핵심은 검증 함수 둘이 **`src/sim/**` 를 직접 import 해 시뮬 코어를 통째로 번들한다**는 것이다 —
서버가 제출된 리플레이를 그 번들로 재계산해 해시를 대조한다.

## 하위 디렉터리

| 디렉터리 | 역할 | 배포 대상? |
|---|---|---|
| `verify-invasion/` | 침공 3레이어 리플레이 재실행 검증 + 위조 거부 | **예** — `src/sim/**` 변경 시 재배포 필수 |
| `verify-commission/` | 의뢰 런 검증 + 확정 지급물 발급 원장 | **예** — `src/sim/**` 또는 `src/run/commission*` 변경 시 |
