...
--
-- ⚠️ **이 파일은 처음에 `20260727010000` 으로 만들어졌다가 `20260727011000` 으로 개명됐다.**
--    같은 날 다른 레인(ADR-0038 행성 인기 배율)이 `20260727010000_planet_popularity.sql` 을
--    먼저 올려 **version 이 충돌**했기 때문이다. `schema_migrations.version` 은 기본키라
--    적용 기록 insert 가 `on conflict (version) do nothing` 으로 **조용히 누락**된다 —
--    SQL 은 돌았는데 원장에는 다른 레인 이름만 남는 형태라 사후 추적이 불가능해진다.
--    **같은 날 두 레인이 마이그레이션을 만들 때는 version 을 분 단위까지 벌려라.**
--
-- 후행 `20260727020000_invasion_band_restore.sql` 이 이 파일의 보스 배정을 **포함해**
-- 20행 레이아웃을 통째로 다시 쓰므로, 원격에는 **후행 하나만 적용해도 최종 상태가 된다**.
-- 이 파일은 그 사이 상태의 기록이자 근거로 남긴다.
--
