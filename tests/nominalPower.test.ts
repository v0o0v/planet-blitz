...
describe('표준 빌드 점프 — 하네스 치트 패널이 기대는 계약', () => {
   ...
   */
  /**
   * ⚠️ **§R51(2026-08-08) 이후 정정 — 「명목표가 후반 성장을 못 본다」는 이제 *선택*이다.**
   * 조종사 레벨 성장 배율(`loadout.ts` `PILOT_LEVEL_GROWTH_PER_LEVEL`)이 생겨 실제 런에는
   * 후반 성장이 있다. 명목표는 그것을 **일부러 안 태운다** — `computeLoadoutStats` 의 `level`
   * 인자를 안 넘기기 때문이고, 이 표의 용도가 «섀시가 서로 얼마나 다른가» 이기 때문이다.
   * 그 배율은 **전 기체 공통**이라 `powerSpread`·`relativeToStriker` 를 정확히 보존하므로,
   * 태우든 안 태우든 기체 균형 판정은 같다. 태우지 않는 쪽이 표를 레벨 축에서 자유롭게 둔다.
   */
