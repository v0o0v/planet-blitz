...
describe('자원 결 · 스트림 불변', () => {
  // ⚠️ **종전 이 절은 «`id 15`+`id 19` 를 실은 런의 세 스트림이 600틱 뒤에도 같다» 였고, 그
  //    단언은 처음부터 참이 아니었다** (2026-08-08 출시 전 밸런스 레인에서 적발). 근거:
  //
  //     · `id 19` 는 엘리트 처치 자리에 **광석 덩어리를 세운다**. 그 광석은 `destructible` 이지만
  //       `isObjectiveDestructible` 이 참이라 **자동 조준 대상**이다(그렇게 설계됐다 —
  //       `catalyst/resource.ts` §id 19 가 "전부가 조준 대상이라" 를 사유로 잡몹을 뺐다).
  //       조준이 광석으로 한 번 새면 그 뒤 처치·드랍 시점이 통째로 밀린다. 즉 **`id 17` 의
  //       소환을 이 절에서 뺀 바로 그 사유가 `id 19` 에도 그대로 있었다.**
  //     · 실측(시드 `0xca7a`·무입력·본 워크트리): 광석이 **t=258** 에 서고, 처치 수가 t=462 에
  //       갈리고(36 vs 33 @3000틱), `dropRng` 가 **t=581** 부터 갈린다.
  //     · **반사실 대조** — 이 레인의 밸런스 계수를 전부 되돌려도(`ENEMY_FIRE_RATE_MULT` 1.0)
  //       광석은 t=246 에 서고 `dropRng` 는 **똑같이 t=581 에 갈린다.** 옛 단언이 초록이었던 것은
  //       그 설정에서 **t=600 이 우연히 재수렴한 틱**이었기 때문이다(스트림은 갈렸다 붙었다 한다).
  //       밸런스가 움직이자 그 동전 던지기가 뒤집혔을 뿐, 회귀가 생긴 것이 아니다.
  //
  //    그래서 계약을 **참인 것으로** 좁혀 다시 쓴다. 지키려던 것(`bench/runCurve.ts` 의 짝지은
  //    base↔cat 배수가 오염되지 않을 것)은 여전히 지켜진다 — 광석이 만드는 차이는 **카드의 힘
  //    그 자체**라 배수에 들어가는 것이 맞고(runCurve 헤더가 조향 정책 사례로 같은 구분을 이미
  //    적어 뒀다), 오염이 될 축은 «촉매 경로가 스트림을 밀어 버리는 것» 뿐이다. 그 축은 카드별
  //    «RNG 미소비» 절(위 §id 15·§id 17·§id 19)과 아래 두 테스트가 잡는다.

  it('`id 15` 를 실은 런은 3000틱 뒤에도 무촉매 런과 비트 동일하다', () => {
    // `id 15` 는 새 난수를 굴리지도, 조준 대상 엔티티를 세우지도 않는다(떨구는 결정은
    // `kind === 'loot'` 라 `isPlayerTargetable` 밖이다). 그래서 이 카드만은 **런 전체**에
    // 스트림 불변을 걸 수 있다.
    // ⚠️ 무입력 런에는 적립 사건(보급 습격)이 없어 이 창에서 카드가 **발동하지 않는다.**
    //    그러므로 이 테스트가 못 박는 것은 "발동해도 안 민다" 가 아니라 **"실어 두는 것 자체의
    //    비용이 0"** 이다(게이트가 조용히 거동을 바꾸지 않는다). 발동 경로의 미소비는 위
    //    §id 15 절의 «RNG 미소비» 가 앵커를 직접 불러 따로 잡는다 — 둘은 서로를 대신하지 못한다.
    const a = w();
    const b = w([CARD_EXTRACTION]);
  ...
  it('`id 19` 는 광석이 서기 전까지 스트림을 안 민다 — 그 뒤의 갈림은 조준 대상이 는 결과다', () => {
    // 두 단언이 한 쌍이다:
    //  ① 광석이 서기 **전** 구간은 매 틱 비트 동일하다 → 카드를 실었다는 사실만으로 스트림이
    //     밀리지는 않는다(순수 미소비 증명 — 아직 세계가 안 갈렸으므로 다른 해석이 없다).
    //  ② 광석은 **조준 대상**이다 → ①의 구간을 런 전체로 넓힐 수 없는 이유가 여기 있고,
    //     그것은 결함이 아니라 카드의 힘이다.
    // 틱 수·스폰 시각을 리터럴로 박지 않는다 — 밸런스를 만질 때마다 빨개지던 형태다.
    const a = w();
    const b = w([CARD_MOTHERLODE]);
    const oreOf = (s: WorldState): Entity | undefined =>
      s.entities.find((e) => !e.dead && e.kind === 'destructible' && e.ownerId === CATALYST_ORE_MARK);

    let ore: Entity | undefined;
    let ticks = 0;
    for (; ticks < 3000; ticks++) {
      stepWorld(a, IDLE);
      stepWorld(b, IDLE);
      ore = oreOf(b);
      if (ore !== undefined) break;
      // ① 광석이 서기 전에는 한 틱도 밀리면 안 된다.
      expect(rngStates(b), `t=${ticks} 에서 광석 없이 스트림이 밀렸다`).toEqual(rngStates(a));
    }

    // 비어 있는 창이면 위 루프는 아무것도 안 잰다 — 그 상태를 초록으로 두지 않는다.
    expect(ore, '3000틱 안에 광석이 한 번도 서지 않았다 (엘리트 처치가 창 밖으로 밀렸다)').toBeDefined();
    // ② 갈림의 메커니즘을 못 박는다. 이 술어가 거짓이 되면 위 §계약 주석이 통째로 낡는다.
    expect(ore !== undefined && isObjectiveDestructible(ore)).toBe(true);
  });
