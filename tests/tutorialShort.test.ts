...
describe('튜토리얼 단축판 (maxSegments)', () => {
  ...
  //
  // ⚠️ 2026-07-27 밸런스 패스에서 `60 * 200` → `60 * 600` 으로 넓혔다. 적 축의
  // `SEGMENTS.killGoal` 합계가 80 → 240 으로 3배가 되면서(ADR-0037 · Lane D) 내구 파일럿이
  // 7칸 궤적을 완성하는 데 **23,457틱**이 걸린다(실측 — 런 자체는 23,669틱에 승리로 끝난다).
  // 구 상한 12,000틱에서는 궤적이 `[0,1,2,3]` 에서 잘렸다. **관측 예산만** 넓힌 것이고 단언
  // (풀 런 순서 보존)은 한 글자도 바뀌지 않았다.
  const ampleTicks = 60 * 600; // 여유 상한 — 실제 점프는 처치 할당 달성 시 발생.
