...
describe('항목 3 · 돌진 예고 듀티 (실 sim · 실전투 구간)', () => {
   ...
   *
   * ## 7차 (2026-08-06, ADR-0051): **파일럿의 생존을 관측에서 뺐다**
   *
   * 1~6차가 매번 수치를 다시 맞춰야 했던 진짜 이유가 여기서 드러났다 — 표본이 차저 거동이
   * 아니라 **봇이 얼마나 버티는가**로 정해지고 있었다. 실측(2026-08-06, 16시드):
   *
   * | | 표본 있는 셀 | 최소 표본 | 중위 | 최댓값 |
   * |---|---|---|---|---|
   * | 6차 구성(사멸 파일럿 · 창 120틱) | **9 / 16** | **9** | 10.84% | **100.00%** |
   * | 7차 구성(내구 파일럿 · 창 1500틱) | **16 / 16** | **3,115** | 9.01% | **18.21%** |
   *
   * 사멸 구성의 표본 0 셀 7개는 전부 `gameOver` 로 창에 도달조차 못 한 셀이고(봇은 467~840틱에
   * 죽는다), 최댓값 100% 는 **표본 9건짜리 착시**다. 그 격자 위에서 중위·최댓값을 계약으로
   * 굳히니 밸런스를 만질 때마다 빨개진 것이다.
   *
   * 그래서 `invasionIntegration.test.ts` 와 **같은 수법**을 쓴다 — 내구 파일럿으로 "관측 구간에
   * 도달하는 것"을 보장하고 단언은 약화하지 않는다. 내구화는 측정 대상을 왜곡하지 않는다:
   * 이 계약이 재려는 것은 "예고를 필요로 하는 사람이 보는 화면"이고, **살아서 계속 싸우는
   * 파일럿이 곧 그 사람**이다. 죽어 가는 봇이 아니라.
   */
  /** 내구 파일럿 — 사망을 관측에서 뺀다(위 7차 절). 조준·거동은 기본값 그대로다. */
  const DURABLE_PILOT = { ...DEFAULT_CONFIG, playerHp: 1_000_000_000 };

  function commitDuty(seed: number, warmup: number, ticks: number): { commit: number; total: number } {
    const state = createWorld(seed, DURABLE_PILOT);
  ...
  /** 워밍업 — 차저가 붙는 실전투 구간까지 밀어 준다(2차 반려 사유). */
  const WARMUP = 300;
  /**
   * 관측창 1,500틱. **창을 셋(300/600/900 × 120틱)에서 하나로 바꾼 것이 7차의 핵심**이다.
   *
   * 구 구성은 짧은 창 셋을 나열해 "한 창이 수치를 만드는" 것을 막으려 했지만, 봇이 467~840틱에
   * 죽으므로 뒤쪽 두 창은 **관측이 아니라 사망 여부**를 재고 있었다. 내구 파일럿으로 바꾸면
   * 창을 나눌 이유가 없다 — 한 개의 긴 창이 같은 구간을 전부 덮고, 셀마다 3,000건 이상을 모아
   * 표본 착시(6차의 "표본 9건에 듀티 100%")가 구조적으로 불가능해진다.
   */
  const WINDOW = 1500;
  ...
  /** 한 번 돌리고 두 단언이 나눠 쓴다(16런은 다시 돌리면 아프다). */
  let cells: Cell[] = [];
  beforeAll(() => {
    cells = SEEDS.map((seed) => ({ seed, ...commitDuty(seed, WARMUP, WINDOW) }));
  }, 600000);

  /**
   * ## 7차의 게이트 — 상한은 **살렸고**, 분포 통계는 내렸다 (2026-08-06, ADR-0051)
   *
   * 구 단언 넷 중 **셀별 상한만 남긴다.** 상한은 밸런스 수치가 아니라 **가독성 계약**이다 —
   * "예고가 배경이 되면 예고가 아니다"는 이 저장소가 실제로 신고를 받은 축이고, 내구 파일럿
   * 위에서는 표본이 봇 실력에서 떨어져 나와 결정론적으로 성립한다(실측 최댓값 18.21%).
   *
   * 내린 셋과 이유:
   *  - **중위 10%** — 실측 9.01% 로 여유가 1.1배뿐이다. "드물다"의 정도는 연출 밸런스라
   *    출시 직전 패스에서 함께 잡는다(ADR-0051 §3). 상한이 남아 있으므로 "전 셀 39%" 류
   *    폭주는 여전히 잡힌다.
   *  - **표본 0 셀 수** — 내구 파일럿에서는 항상 0 이라 아무것도 재지 않는다(항진 단언).
   *  - **커밋 0 셀 비율** — 아래 ②가 같은 축을 더 직접적으로 본다.
   */
  it('어떤 시드에서도 커밋 듀티가 30% 를 넘지 않는다(예고가 배경이 되지 않는다)', () => {
    // ① 공허 가드 — 셀마다 표본이 충분하다. 내구 파일럿이라 이 하한은 **봇 실력이 아니라
    //    차저 출현**만 반영한다(실측 최소 3,115건). 카드 풀이 바뀌어 차저가 사라지면 여기서 잡힌다.
    for (const c of cells) {
      expect(c.total, `시드 ${c.seed} 차저 표본 ${c.total}건 — 웨이브 카드 풀을 먼저 봐라`).toBeGreaterThanOrEqual(500);
    }
    // ② 상한. 실측 최댓값 18.21% 에 1.65배 여유. 2차 코드의 52.92% 는 여기서 여전히 빨개진다.
    const worst = cells.reduce((a, c) => Math.max(a, c.commit / c.total), 0);
    expect(worst, `최악 셀 듀티 ${(worst * 100).toFixed(2)}%`).toBeLessThanOrEqual(0.3);
  });

  it('돌진 예고가 실 sim 에서 실제로 뜬다(배선·조준 술어 생존 신호)', () => {
    // 상한만 걸면 "예고가 아예 안 뜨는" 방향으로는 항상 통과한다 — 그건 기능 소실이다.
    // `COMMIT_RANGE` 400→260 이면 4차 실측에서 커밋 0 셀이 6 → 17/33 이 됐다(그 방향의 회귀).
    const speaking = cells.filter((c) => c.commit > 0).length;
    expect(speaking, '커밋 예고를 내는 시드가 없다 — 기능이 통째로 죽었다').toBeGreaterThanOrEqual(3);
  });
});
