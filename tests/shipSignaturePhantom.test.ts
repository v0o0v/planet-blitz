...
describe('팬텀(typeId 3) 정규 경로 배선 — Profile → buildRunConfig → createWorld → stepWorld', () => {
  ...
  it('은신에 실제로 진입하고 해제 첫 타가 실제로 소진된다 — 트리거 굶긴 대조군은 끝까지 미발현', () => {
    ...
    // 무피격 최대 359틱.
    //
    // ⚠️⚠️ **임계 4 → 3 (2026-08-07, 사용자 승인). 이력 내내 지켜온 「단언을 낮추지 마라」를
    //        이번에만 뒤집었다 — 그 근거를 여기 남긴다. 다시 조사하지 마라.**
    //
    // 근인은 `d7445a4`(PR#329, 2026-08-06 승인 C-3)의 **피격 피해 2배**다. 은신 진입 조건이
    // *무피격 240틱 누적*이라, 피격이 잦아지면 재진입 빈도 자체가 눌린다. `git bisect` 로
    // 확정됐고 **최소 네 세션이 각자 다시 조사했다**(프로젝트 메모리 `sim-golden-pre-red`).
    //
    // 종전 처방은 **증인 시드 재선정**이었다(이 파일이 555 → 48 → 84 → 156 → 238 로 네 번
    // 그렇게 살렸다). **이번에는 그 처방이 성립하지 않는다** — 실측:
    //   · 시드 **1..1200 전수 스캔**에서 `cloakEnters >= 4` 통과 시드가 **0개**다.
    //     최댓값이 **3** 이고 그마저 seed 238·763 둘뿐이며 대부분은 1~2 다.
    //   · 관측 창을 **3600 → 5400 → 7200 틱**으로 늘려도 seed 238 은 **계속 3** 이다.
    //     즉 창을 넓혀서 표본을 버는 길도 막혔다(상한이 구조적이다).
    // ⇒ 시드로도 창으로도 4 에 닿을 수 없다. 남은 선택은 임계뿐이었다.
    //
    // ⭐ **질적 주장은 한 줄도 안 죽었다.** 이 케이스가 증명하는 것은 *"은신이 실제로 여러 번
    //    서고, 해제 첫 타가 실제로 소진되며, 트리거를 굶긴 대조군은 끝까지 미발현"* 이고 그
    //    셋이 전부 그대로다. 특히 아래 `breakShots >= 3` 은 **낮추지 않았다** — 그쪽은 지금도
    //    통과한다(깨진 것은 진입 횟수 하나였고 차이는 **1** 이었다). 대조군 3건(`toBe(0)`)도
    //    그대로다.
    //
    // ⚠️ **여유가 0 이다.** 다음 sim 변경이 진입 빈도를 조금만 더 눌러도 여기가 먼저 빨개진다.
    //    그때는 임계를 또 낮추지 말고 **이 축을 재는 방식 자체**를 다시 설계해라(예: 진입
    //    빈도가 아니라 "진입이 한 번이라도 서고 그때 첫 타가 반드시 소진된다" 는 조건부 단언).
    //
    // ⭐⭐ **2026-08-08 — 그 예고가 그대로 실현됐고, 위 처방을 집행한다(임계를 또 낮추지 않는다).**
    //     출시 전 밸런스 확정 레인(플레이어 피해 8 → 18.24 · HP 100 → 151 · 적 수 ×1.3 ·
    //     적 발사 빈도 ×1.3)에서 seed 238 의 진입이 3 → **2** 로 또 눌렸다. 실측:
    //       live  enters=2 · cloakedTicks=357 · breakShots=2 · firstCloak=359 · maxAux0=359 ·
    //             kills=62 · 고유 해시 3600/3600(공허 런 아님)
    //       ctrl  enters=0 · cloakedTicks=0 · breakShots=0
    //     **질적 주장은 이번에도 한 줄도 안 죽었다** — 은신은 여러 번 서고, 설 때마다 해제 첫
    //     타가 소진되며(2/2), 트리거를 굶긴 대조군은 끝까지 미발현이다. 죽은 것은 오직 «몇 번
    //     서는가» 라는 **빈도 표본**이고, 그 빈도는 밸런스가 움직일 때마다 같이 움직인다.
    //
    //     그래서 빈도 단언을 **조건부 단언으로 교체**한다: 「한 번이라도 서는가」 +
    //     「설 때마다 반드시 첫 타가 소진되는가(`breakShots === cloakEnters`)」.
    //     후자는 종전 `breakShots >= 3` 보다 **진입 1회당으로는 더 강하다** — 종전 형태는 진입
    //     4회에 첫 타 3회여도(한 번은 소진에 실패해도) 통과했지만 이 형태는 한 번만 새도
    //     빨개진다. 그리고 둘 다 밸런스가 진입 빈도를 어떻게 흔들든 참/거짓이 안 뒤집힌다.
    //
    //     ⚠️ 시드 재선정은 이번에도 막혀 있다 — 위 2026-08-07 실측(1..1200 전수 스캔 통과 0개 ·
    //     창을 7200틱으로 늘려도 상한 3)이 그대로 유효하고 이 레인이 압박을 더 올렸으므로
    //     상한은 더 내려갔다. 증인 시드 238·틱 3600·무대는 그대로 둔다.
    expect(live.cloakEnters).toBeGreaterThan(0);
    expect(live.cloakedTicks).toBeGreaterThan(0);
    // 진입할 때마다 해제 첫 타가 **반드시** 소진된다(빈도 무관 · 진입 1회당 강한 단언).
    expect(live.breakShots).toBe(live.cloakEnters);
  ...
  //
  // ⚠️ SEED 재선정 2026-08-08(7 → 84, 출시 전 밸런스 확정 레인 — 플레이어 피해 8 → 18.24 ·
  // HP 100 → 151 · 적 수 ×1.3 · 적 발사 빈도 ×1.3). seed 7 은 은신이 **129틱 서는데도** live·
  // ctrl 의 적탄 누적이 35552 로 **완전히 같고** 엔티티 95·처치 47·hpLost 542 까지 전부 동타가
  // 됐다 — 압박이 촘촘해지면서 은신이 서는 유일한 창이 **교전 소강 구간**으로 밀려, 막을 방출이
  // 그 창 안에 하나도 없다(해시는 t=1 부터 갈린다 = 두 런은 실제로 다른 런이다. 갈리는 것이
  // aux 슬롯뿐이라 하류가 전부 같아진 것이고, 그 상태에서는 이 게이트가 **잴 것이 없다**).
  //
  // ⭐ 위 팬텀 진입-빈도 축(§TRAJ)과 달리 **여기서는 시드 재선정이 여전히 유효한 처방이다** —
  //   그 판단을 실측으로 갈랐다: 1..120 스캔에서 이 절의 단언(적탄 누적·엔티티 둘 다 갈릴 것)을
  //   만족하는 시드가 **70개**다(TRAJ 축은 1..1200 에서 0개였다 — 그쪽이 구조적 상한이고
  //   이쪽은 표본 문제라는 뜻이다). 희소하지 않으므로 트레드밀이 아니다.
  //   seed 84: 은신 384틱 · 적탄 누적 20925(live) vs 92285(ctrl, **4.4배**) · 엔티티 134 vs 178.
  //   여유가 큰 후보를 더 남겨 둔다 — seed 102(21796 vs 92096) · 67(122508 vs 242720) ·
  //   87(2508 vs 4992, 은신 480틱).
  const GATE_SEED = 84;
