 ...
 *
 * ## ⚠️ 2026-08-04 재생성 #2 — 카르곤 정예 2종(카드 풀 8 → 10)
 * 카르곤에 엘리트 2종(용암 포대·용암 거인)이 생기며 `CARD_POOL` 이 10장이 됐다. 카드 추첨은
 * **풀 길이로 인덱스를 뽑으므로** 길이가 바뀌면 같은 시드라도 뽑히는 카드열이 통째로 갈린다 —
 * 카르곤 6런이 전부 발산했고 베르단 6런은 자기 풀을 쓰므로 **한 글자도 안 움직였다**(재생성
 * 로그로 확인: berdan-engage/* final 해시 불변). 즉 이 재생성은 "카르곤 웨이브 구성이 바뀌었다"
 * 를 그대로 반영한 것이고, 스킬 슬라이스·loadout·폴드 레이아웃 축은 건드리지 않았다.
 *
 * ⚠️ 재생성 직전 이 파일은 **깨끗한 트리에서 34건 전부 초록**이었다(stash 대조로 확인) —
 * 즉 이번 15건 실패는 쌓인 빚이 아니라 이 변경이 만든 것이고, 그래서 같은 회차에 갚는다.
 *
 * ## ⚠️ 2026-08-04 재생성 — 발사 간격 고정소수점화 (5레인 통합)
 * `fireCooldown`(정수 틱) → `fireCooldownQ`(1/256틱 고정소수점)으로 바뀌었다(밸런스 큐 §R39).
 * `autoAttack` 이 쿨다운을 매 틱 `FIRE_CD_Q` 씩 깎고 발사 시 잔여분을 보존한 채 간격을 더하므로
 * **틱 1 부터** 해시가 발산한다(실측: `profile-path/firepower` 골든 0xab7c9d3a ≠ 현재 0xf269e06).
 * 이 파일의 대조 축(`BASELINE_BUILDS` 스킬 투자 · 트리 슬라이스 가중)은 **한 글자도 안 바뀌었다** —
 * 갈린 것은 sim 산술이고, 그래서 재생성이 맞다.
 *
 * ⚠️ **부채가 섞여 있다.** 재생성 직전 이 파일은 `074ec88`(main)에서 **이미 27건이 깨져 있었다** —
 * main 이 그 뒤로 sim 을 여러 차례 정당하게 바꾸면서(밸런스 큐 §R11~§R17) 골든을 그때마다
 * 재생성하지 않아 쌓인 빚이다. 이번 재생성이 그 빚까지 함께 갚는다. **다음 sim 변경 때는 같은
 * 회차에 재생성해라** — 오래 깨진 골든은 진짜 회귀와 구분되지 않는다.
 *
 * ## ⚠️ 2026-07-28 재생성 — 공간 해시 broad-phase 수정 (`fix/spatial-hash-large-radius-broadphase`)
 * `src/sim/collision.ts` 의 `SpatialHash` 가 엔티티를 **중심 셀 한 칸**에만 등록하면서 질의는
 * **탐침 반경**으로만 셀을 훑어, 접촉 조건(`중심거리 <= 탐침반경 + 엔티티반경`) 중 **엔티티
 * 반경만큼의 띠가 broad-phase 에서 통째로 빠져 있었다.** 적·탄이 셀 경계 너머에서 판정을
 * 흘리고 있었다는 뜻이라, 수정하자 명중 틱이 달라져 **12런 전부** 해시가 갈렸다
 * (`profile-path/firepower` 는 틱 23, `profile-path/no-invest` 는 틱 72 부터 발산).
 *
 * 이 파일의 대조 축(`BASELINE_BUILDS` 스킬 투자 · 트리 슬라이스 가중)은 **한 글자도 안 바뀌었다** —
 * 갈린 것은 sim 판정이고, 그래서 재생성이 맞다. 상세·분리 실측은
