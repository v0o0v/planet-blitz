...
describe('버블 방막 — 순수 산술 골든', () => {
  it('흡수분 + 잔여분 = 원래 피해 (막은 총량 흡수지 배율이 아니다)', () => {
    for (let d = 0; d <= 200; d++) {
      for (const s of [0, 1, 17, filmCapacityFor(100)]) {
...
/**
 * 관측 런의 막 만재 내구 = 기본 플레이어 HP(100) 기준값. 생존용 `DURABLE_HP` 와 분리하는
 * 사유는 `runObserved` 안의 대입 지점 주석이 정본이다.
 */
...
interface Observed {
  ...
  maxAux1: number;
  /**
   * 이 런의 막 만재 내구(`WorldState.filmCapacity`). 내구는 이제 **최대 HP 비율**이라
   * (2026-08-08 `FILM_ABSORB_HP_BP`) 상수로 단언할 수 없다 — 이 무대는 `DURABLE_HP` 를
   * 쓰므로 값이 기본 런과 다르다. 그래서 상한을 세계에서 직접 받아 온다.
   */
  filmCapacity: number;
  hashes: number[];
}
...
function runObserved(
  ...
  // ⚠️ 막 내구를 **기본 HP 기준값으로 되돌린다**(2026-08-08 개정 대응).
  //
  // 내구는 이제 최대 HP 비율이라(`FILM_ABSORB_HP_BP`), 관측 런이 쓰는 `DURABLE_HP` 를 그대로
  // 두면 막 1장이 수천만이 되어 **한 번도 소진되지 않는다** — 소진·파열·밀어내기 배선이 전부
  // 관측 불가가 된다(실측: bursts 0). 이 스위트가 재는 것은 *배선이 사는가*이지 *내구가 얼마인가*
  // 가 아니므로, 생존용 HP 와 관측용 내구를 **분리**한다. 크기 축은 `bench/nominalPower.ts` 소관.
  ...
  return {
    ...
    maxAux0,
    maxAux1,
    filmCapacity: state.filmCapacity,
    hashes,
  };
}

describe('버블(typeId 6) 정규 경로 배선 — Profile → buildRunConfig → createWorld → stepWorld', () => {
  // 무대 선정도 계약의 일부다: 막 1장의 흡수량보다 누적 피해가 작으면 신호가 묻힌다.
  // p2/t2 는 신호가 크다. (내구는 이제 최대 HP 비율이라 무대가 `DURABLE_HP` 를 쓰는 이상
  // 흡수량도 그만큼 커진다 — 그래서 아래 대조군 단언이 `filmCapacity` 를 직접 읽는다.)
  ...
  it('방막이 실제로 발현한다 — 트리거 굶긴 대조군보다 피해가 작고 막 내구가 소모된다', () => {
    ...
    expect(ctrl.hpLost).toBeGreaterThan(live.filmCapacity);
    ...
    // 런타임 상태가 실제로 굴러간다: 막이 서고(aux0 = state.filmCapacity) 재생 타이머가 돈다.
    expect(live.filmCapacity).toBe(OBSERVE_FILM_CAP);
    expect(live.maxAux0).toBe(live.filmCapacity);
