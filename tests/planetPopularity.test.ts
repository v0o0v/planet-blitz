...
function runOnce(seed: number, stage: number, centi?: number, planet = 0) {
...
describe('§6 정규 경로 배선 — 배율이 실제 런에 도달한다', () => {
  ...
  it('배율 1.20 런의 메타 XP 합이 중립 런보다 크다', () => {
    const base = SEEDS.map((s) => runOnce(s, 11).xpTotal).reduce((a, b) => a + b, 0);
    const up = SEEDS.map((s) => runOnce(s, 11, 120).xpTotal).reduce((a, b) => a + b, 0);
    expect(base).toBeGreaterThan(0);
    expect(up).toBeGreaterThan(base);
  });

  it('배율 0.85 런의 메타 XP 합이 중립 런보다 작다', () => {
    const base = SEEDS.map((s) => runOnce(s, 11).xpTotal).reduce((a, b) => a + b, 0);
    const down = SEEDS.map((s) => runOnce(s, 11, 85).xpTotal).reduce((a, b) => a + b, 0);
    expect(down).toBeLessThan(base);
  });

  it('배율은 **런 풀** XP 를 건드리지 않는다(ADR-0036 런 내 리듬 불변)', () => {
    // 같은 시드·같은 입력이면 런 풀(state.xp)은 배율과 무관해야 한다. 단, 드랍 게이트가
    // RNG 를 흔들지 않으므로(소비 1회 고정) 봇 경로가 동일해 직접 비교가 성립한다.
    const a = runOnce(1000, 11);
    const b = runOnce(1000, 11, 120);
    expect(b.state.xp).toBe(a.state.xp);
  });

  it('배율 1.20 이 런당 장비 드랍 **수량**을 늘린다', () => {
    const base = SEEDS.map((s) => runOnce(s, 11).drops).reduce((a, b) => a + b, 0);
    const up = SEEDS.map((s) => runOnce(s, 11, 120).drops).reduce((a, b) => a + b, 0);
    expect(up).toBeGreaterThan(base);
  });
...
describe('§7 배율이 닿으면 안 되는 축', () => {
  it('품질(rarity) 축은 전 행성 동일 — 배율이 등급 분포를 바꾸지 않는다', () => {
    // 같은 시드에서 배율만 바꿔도 **드랍된 각 건의 등급 코드 분포**는 게이트 통과 건수에
    // 비례할 뿐, 등급 threshold 자체는 불변이다. rarity 배율 인자에 planetMult 가 섞였다면
    // 유니크 비율이 체계적으로 달라진다.
    const seeds = [1000, 1001, 1002, 1003, 1004, 1005, 1006, 1007];
    const uniqueShare = (centi?: number) => {
      let n = 0;
      let uniq = 0;
      for (const s of seeds) {
        for (const r of runOnce(s, 11, centi).loot) {
          n++;
          if (r.rarity === 3) uniq++;
        }
      }
    ...
    // 유니크는 희소해 표본 잡음이 크므로 "체계적 상승이 없다"만 본다 — 배율을 rarity 에
    // 곱했다면 1.20 에서 유니크 비율이 눈에 띄게 올라간다.
    expect(Math.abs(uniqueShare(120) - uniqueShare(100))).toBeLessThan(0.05);
  });
