 ...
 *   - ⛔ **새로 생김** — 위 수정이 난이도 곡선을 통째로 천장에 붙였다(24시드 하위 100 ·
...
describe('밸런스 스모크 — 밴드별 클리어율', () => {
  ...
  // 24시드 실측(2026-07-27, 4세대 = 실드 국면 코어 관통 수정 이후):
  //   하위 **100.00%** · 중하 **99.40%** · 중위 **90.97%**.
  // 기지별 중하 #08~#14 = 100/100/100/100/100/100/95.8 · 중위 #15~#20 = 66.7/83.3/95.8/100/100/100.
  //
  // ⚠️ **상한 셋은 지금 사실상 죽어 있다.** 곡선 전체가 천장에 붙어 버렸기 때문이지 상한을
  // 느슨하게 잡아서가 아니다(헤더 ②-4세대). 이 파일이 계속 잡아 주는 것은 ⓐ클리어 가능성
  // ⓑ밴드 순서(하위 > 중위) ⓒ**배선 생존** 셋이고, 그중 ⓒ가 본체다 — 배선이 끊기면 승률이
  // 0 으로 내려앉아 **하한**에 걸린다. 상한 감도를 되찾는 것은 난이도 자체를 되돌리는
  // 일이라 침공 난이도 레인 몫이다(헤더 ⑤). **목표 55~80 / 25~55 는 그대로 둔다.**
  it.each([
    ['하위', 85, 100],
    ['중하', 55, 100],
    ['중위', 25, 95],
...
describe('밸런스 스모크 — 밴드 안 분산', () => {
  it('기지별 클리어 시간 편차가 상한 안(밴드 안에서 판 길이가 널뛰지 않는다)', () => {
    ...
    // 섞였다는 뜻이다. 24시드 실측(2026-07-27, 4세대): 하위 **1448** / 중하 **356** /
    // 중위 **853**틱. (3세대는 1609 / 392 / 893, 구 12시드는 1656 / 565 / 641 이었다 —
    //  상한은 세 세대 내내 그대로다. 여유가 충분해 재조정할 근거가 없다.)
  ...
  it('기지별 승률 편차가 상한 안', () => {
    ...
    //
    // 24시드 실측(2026-07-27, 4세대): 하위 **0.00** · 중하 **1.46** · 중위 **12.36** pp.
    // 기지별 승률 #15~#20 = 66.7 / 83.3 / 95.8 / 100.0 / 100.0 / 100.0.
    // (3세대는 3.76 / 12.93 / **31.70**, #16 이 4.2% 로 홀로 분산을 만들었다. 4세대 수정이
    //  #16 을 80.2%(96시드)로 끌어올려 그 출처가 사라졌다 — 아래 "출처는 #16 단독" 문단은
    //  이제 **역사 기록**이고, 상한 33 은 실측 12.36 대비 여유가 과도하다. 다만 난이도를
    //  되돌리면 분산이 되살아나므로 상한은 건드리지 않는다.)
    //
...
// #16=상한** 이다.
//
// ## 24시드 실측 (2026-07-27, 4세대 = 실드 국면 코어 관통 수정 이후 · 기체 1~6 순)
//   #12 = 100.0 / 100.0 / 100.0 / 87.5 / 100.0 / 100.0   (87.5~100.0)
//   #16 =  87.5 /  87.5 /  91.7 / 70.8 /  91.7 /  87.5   (70.8~ 91.7)
//
// 직전(3세대) 값은 #12 = 66.7~100.0 · #16 = 8.3~29.2 였다. 위로 크게 움직인 원인은 밸런스가
// 아니라 **결함 제거**다 — `.omc/research/invasion-powerup-inversion-2026-07-27.md` 가 기록한
// "기지 #12 에서 공격 파워업이 승률을 깎는다"의 근인이 `src/sim/world.ts` 아군탄 명중 루프에
// 있었다: 실드 발생기 국면의 코어(`timer === 1`)가 조준에서만 빠지고 **충돌에는 남아** 반지름
// 90 짜리 무적 차폐물로 작동했다. 자세한 것은 `.omc/research/invasion-core-shot-block-2026-07-27.md`.
//
// ⚠️ **#16 hi 70 → 96.** 상향 담당은 여전히 #16 이고 감도도 살아 있지만 **여유가 1눈금뿐**이다
// (24시드 최소 눈금 4.17pp). 같은 트리에서 재실증한 값이다:
//     `data/ships/bruiser.ts` baseBp `{20000,…}` : #16 기체 1 = 87.5 → **100.0** → hi 96 위반
//     `data/ships/bruiser.ts` baseBp `{-9000,…}` : #12 기체 1 = 100.0 → **0.0** → lo 15 위반
// 기저 최대 91.7 과 변조 100.0 **사이**에 놓인 실현값은 95.8 하나뿐이라, hi 96 이 이 sim 에서
// 확보 가능한 최대 여유다(3세대의 hi 70 도 같은 구조의 1눈금이었다).
//
// ⚠️ **#12 는 상향 감도를 완전히 잃었다**(6기체 중 4기체가 24시드 100.0%). 애초에 하한 담당이라
// 대가가 없다고 적어 뒀지만, 이제는 96시드로 늘려도 회복되지 않는다 — 결함 수정 후 #12 는
// 96시드에서도 100.0%(96/96)다. 침공 난이도가 밴드 목표로 돌아오기 전까지 이 게이트의 상향
// 감도는 **#16 1눈금이 전부**다.
const ROSTER_GATE_BASES = [
  { nn: 12, lo: 15, hi: 100, band: '87.5~100.0' },
  { nn: 16, lo: 5, hi: 96, band: '70.8~91.7' },
] as const;
