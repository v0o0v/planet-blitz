...
describe('밸런스 스모크 — 밴드별 클리어율', () => {
  // ⚠️ TODO(밸런스) — 중위 밴드 목표는 25~55% 였는데 **강제 스크롤 앵커 정책 개정**
  // (2026-07-27, `src/sim/scrollMode.ts`) 이후 실측 76.4% 다. 적·탄이 더 이상 창 뒤로 쓸려
  // 나가지 않아 공격자가 유리해졌다(사용자 승인된 변경). 밸런스 수치 튜닝은 출시 직전 일괄
  // 방침이라 지금은 **실측에 맞춰 상한만 열어 두고** 목표 재배치를 그 패스로 넘긴다.
  // 이 상한은 "이 값이 좋다"가 아니라 "지금 여기 있다"는 기록이다.
  it.each([
    ['하위', 85, 100],
    ['중하', 55, 85],
    ['중위', 25, 80],
  ...
  it('밴드 순서대로 어려워진다(하위 > 중위)', () => {
    ...
    //
    // ⚠️ TODO(밸런스) — 원래는 `하위 > 중하 > 중위` 3단 단조였다. 앵커 정책 개정(2026-07-27)
    // 이후 실측이 하위 90.5 > 중하 71.4 **< 중위 76.4** 로 **중하·중위가 역전**됐다. 이건
    // 시드 기지 난이도 램프를 다시 배치해야 한다는 신호이고, 출시 전 밸런스 패스의 항목이다.
    // 여기서는 역전을 **감추지 않기 위해** 단조 단언을 지우는 대신 양 끝(하위 > 중위)만 남기고
    // 이 주석으로 역전 사실을 못박는다. 밸런스 패스가 램프를 고치면 중간 단언을 되살릴 것.
...
describe('밸런스 스모크 — 밴드 안 분산', () => {
  ...
  it('기지별 승률 편차가 상한 안', () => {
    ...
    //
    // ⚠️ 중위 상한은 ADR-0034(강제 스크롤 ANCHOR 정책)로 **28 → 29 재조정**했다. 플레이어
    // 부분 앵커(70%)로 창 뒤 경계 영구 고착이 사라지면서 침공 난이도가 **재분배**됐고,
    // 중위 편차가 14.164 → 28.767pp 로 벌어졌다:
    //   기지별 승률 #15~#20  33.3/ 8.3/41.7/ 16.7/50.0/33.3  (변경 전, 평균 30.6%)
    //                     →  33.3/25.0/66.7/100.0/16.7/33.3  (변경 후, 평균 45.8%)
    // 주범은 단순 분산 확대가 아니라 **양방향 이동**이다 — #18 이 16.7 → 100.0(+83.3pp),
    // #19 가 50.0 → 16.7(−33.3pp) 로 밴드가 위아래로 찢어졌다. 실패 사유는 세 밴드 모두
    // 시간초과 0 건이라 배선 결함이 아니다(중하는 오히려 30.211 → 28.071pp 로 개선되고,
    // 하위는 전 기지 100% 로 포화해 편차 3.765 → 0.000 이 됐다).
    //
    // 원인은 **적측 앵커 정책 누락이 아니다**: `src/sim/invasion/formation.ts` 는 편대를
    // 창 중심(`runtime.scrollX/scrollY` + AHEAD 오프셋) 기준으로 스폰하고 플레이어 좌표에
    // 의존하지 않는다(파일 헤더가 계약으로 명시). 실제 원인은 **이전 침공 난이도의 일부가
    // 이동 결함에서 나왔다**는 것이다 — 플레이어가 창 뒤 경계에 눌려 있으면 뒤로의 회피
    // 공간이 0 이고 L2 설비 회랑에서 방어포를 피할 여지가 없었다. 앵커로 기동을 회복하니
    // 생존이 쉬워졌다. #19 만 반대로 어려워진 것은 그 기지 배치가 "플레이어가 뒤에 있음"을
    // 전제로 짜였을 가능성이 크다.
    //
    // ⚠️ **#18 은 승률 100% 라 사실상 무저항이다.** 기지 #18·#19 재배치는 출시 전 밸런스
    // 패스 대상이고(defer-balance-tuning · ADR-0034 밸런스 큐 ⑤), 이 주석이 그 큐의 정본
    // 기록이다. 상한 29 는 실측 28.767 기준 **최소값**이며, 이 값을 더 넓히려는 변경은
    // 그때도 기지별 분포를 실측해 원인(난이도 이동 vs 배선 결함)을 판정한 뒤여야 한다.
    const limits: Record<keyof typeof BANDS, number> = { 하위: 20, 중하: 32, 중위: 29 };
    for (const band of Object.keys(BANDS) as (keyof typeof BANDS)[]) {
...
// ⚠️ #12 의 하한은 20 → 15 다(2026-07-27 앵커 정책 개정 후 기체 3 실측 16.7%). 이 게이트가
// 지키는 계약은 **"0%도 100%도 아니다"**(비-스트라이커 기체로도 침공이 성립한다)이지 특정
// 승률이 아니다 — 16.7% 는 6시드 중 1승으로 계약을 만족한다. 수치 재배치는 밸런스 패스 몫.
const ROSTER_GATE_BASES = [
  { nn: 12, lo: 15, hi: 95, band: '50.0~75.0' },
