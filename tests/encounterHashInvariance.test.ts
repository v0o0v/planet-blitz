 ...
 *
 * ## ⚠️ 2026-08-04 기준선 재생성 — 카르곤 정예 2종(카드 풀 8 → 10)
 * 카르곤에 엘리트 2종이 생기며 `CARD_POOL` 이 10장이 됐다. 카드 추첨은 **풀 길이로 인덱스를
 * 뽑으므로** 같은 시드의 카드열이 통째로 재추첨되고, 카르곤 PvE 런 6건의 해시가 발산했다
 * (`RECORD_ENCOUNTER_BASELINE=1` 로 재생성). 이 파일이 지는 축 — "조우 도입이 한 바이트도
 * 새지 않는다" — 은 그대로다. 갈린 것은 웨이브 **내용**이고, 그 변경은 의도된 콘텐츠 추가다
 * (`data/enemies.ts` 카르곤 엘리트 절 · `data/waves.ts` CARD_POOL 머리말 참조).
 */
