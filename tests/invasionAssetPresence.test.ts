 ...
 *
 * ② 침공 배경 3장 — **2026-07-28 해소**. 배경은 엔티티와 달리 `TilingSprite` 라 자산이
 *    **seamless 여야** 해서(아니면 화면 전체에 격자가 뜬다) 오브젝트 생성기를 못 쓴다.
 *    Wang 타일셋의 내부 채움 타일 + 절차 구조선 합성으로 만들었고, 파이프라인은
 *    `node scripts/asset-prep.mjs backdrop` 이 정본이다(이음매를 z-score 로 재고 막는다).
 *
 * ③ 편대·사출 드론 3장 — **2026-07-28 해소**(방어체 25장 직후 이어서 생성).
 */
const KNOWN_MISSING: readonly string[] = [
  // ① 이동 벽이라 스프라이트 슬롯을 타지 않는다 — 영구 예외. 이제 이것 하나뿐이다.
  'def3_fac_press.png',
];
