...
describe('엔티티 해시 레이아웃 계약', () => {
  it('KIND_CODE 는 append-only 다(기존 코드 재배치 금지)', () => {
    ...
    // 28 은 추격·탈출(Lane6) 대피소, 29 는 에코 신호(story Phase D · ADR-0023), 30·31 은 조우
    // 프레임워크(ADR-0033)의 포탈·제단이다. **15 는 영구 결번**이다 —
    ...
    // 금지'이지 '연속'이 아니다). echo(29)·encounterPortal(30)·encounterAltar(31) 은 신규 append 라
    // 이 골든에만 추가되고 기존 코드는 전부 불변이다 — 조우 오브젝트는 조우 런(≈2%)에만 등장하므로
    // 그 외 런의 해시에는 이 코드가 나타나지도 않는다.
