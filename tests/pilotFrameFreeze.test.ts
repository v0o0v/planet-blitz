 ...
 */
/**
 * ## 재동결 이력 — **2026-08-08 (같은 날 7차 · 보스 HP 공비 재조정 · 3/18)**
 *
 * ### 3건(`p2s4242` · `p2s90210` · `p4s4242`)
 *
 * ### 원인 — PR#399 가 깬 전제 위에 서 있던 보스 공비를 다시 정했다
 * `BOSS_HP_GROWTH_PER_STAGE` 1.1761 → **1.2797**. 2차 판 공비는 *"플레이어 화력은 레벨과
 * 무관"* 이라는 전제 위의 값인데 5차 재동결(§R51 조종사 레벨 성장)이 그 전제를 깨서 단계 20
 * 보스 TTK 가 65.4 → 13.1초로 떨어져 있었다. 사용자 결정으로 65초를 복구했다(사유 전문은
 * `src/sim/enemyScale.ts` 의 그 상수 주석 §3차).
 *
 * ⭐ **건수가 진단의 증거다 — 그리고 이번엔 «보스전에 들어간 런» 보다 한 겹 더 좁다.**
 * 이 골든에서 **보스가 스폰되는 런은 4개**(`p2s1` · `p2s4242` · `p2s90210` · `p4s4242`)이고,
 * 그중 **보스를 실제로 깎아 승리한 3개만** 갈렸다. `p2s1` 은 3,000틱 시점에 보스가
 * **만피 그대로**(38,780/38,780)라 HP 값이 입력 열로 되먹임되지 않아 바이트 동일이다.
 * 나머지 14 는 보스에 아예 닿지 못한다. 여기서 18/18 이 갈렸다면 진단이 틀린 것이다.
 *
 * ⚠️ **3·4차 재동결은 같은 축에서 2건이었다.** 집합이 늘어난 것은 진단이 틀려서가 아니라
 * **기준선이 그 사이에 움직였기 때문**이다 — 5차(레벨 성장)·6차(사선 확보)가 봇의 궤적을
 * 바꿔 `p4s4242` 가 이제 3,000틱 안에 보스를 잡는다(**틱 2,996**에 승리 — 창 끝에 걸쳐 있다).
 * ⇒ 다음 재동결에서 이 집합이 또 달라져도 먼저 볼 것은 «어떤 런이 보스를 잡는가» 이지
 *   «건수가 2인가» 가 아니다.
 *
 * ⭐⭐ **교환 대조로 확정했다(재생성 *전에*).** 공비만 `1.1761` 로 되돌려 돌리자 **18/18 이
 * 전부 6차 값 그대로 그린**이었다(발산 0). 원인은 이 상수 하나뿐이고 숨은 회귀는 없다.
 * **vitest 실패 덤프는 파싱하지 않았다** — 같은 `frameStreamHash` 를 복제한 별도 스크립트로
 * 18건 전량을 다시 냈고, 그 김에 «보스 스폰·보스 HP·승패» 를 함께 찍어 위 서명을 실증했다.
 *
 * ⚠️ 단계 1 무대 골든은 이번에도 원리적으로 안 갈린다(곡선이 정확히 ×1).
 */
/**
 * ## 재동결 이력 — **2026-08-09 (§R57 · 적 피해 단계 곡선 신설)**
 *
 * ### 4건(`p1s90210` · `p2s1` · `p4s1` · `p4s90210`)
 *
 * ### 원인 — 얕은 단계(1~11)에 적 피해 배수 2 를 걸었다
 * `stageDamageMult`(`data/waves.ts`) 신설 + `enemyDamageScale`(`src/sim/enemyScale.ts`) 배선.
 * 이 골든은 `LEVEL = 40` → **단계 8** 이라 배수가 정확히 2.0 인 구간을 돈다.
 *
 * ⭐ **건수가 진단의 증거다.** 적 피해는 스폰·적 위치·웨이브 추첨을 **한 글자도 안 바꾼다** —
 * 바뀌는 것은 선체 HP 뿐이다. 그래서 입력 열이 갈리는 런은 «3,000틱 안에 실제로 충분히 맞아
 * 파일럿의 판단(또는 런 종료 틱)이 달라진 런» 뿐이고, 그것이 **4/18** 이다. 여기서 18/18 이
 * 갈렸다면 진단이 틀린 것이다(그 서명은 스폰·입력층 전면 변경의 것이다 — 위 절들 참조).
 *
 * ⭐⭐ **교환 대조로 확정했다(재생성 *전에*).** `ENEMY_DAMAGE_STAGE_PEAK` 만 2 → 1 로 되돌려
 * 돌리자 `pilotFrameFreeze` 18/18 과 `enemyStasis` 해시가 **전부 옛 값 그대로 그린**이었다
 * (발산 0). 원인은 그 상수 하나뿐이고 숨은 회귀는 없다. 값은 **vitest 실패 덤프가 아니라**
 * 같은 시나리오를 재현하는 별도 스크립트로 18건 전량을 다시 떴다.
 */
