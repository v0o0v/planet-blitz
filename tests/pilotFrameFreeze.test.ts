 ...
 */
/**
 * ## 재동결 이력 — **2026-08-08 (§R50 경제 지표 복구 · 원인 둘 · 10/18)**
 *
 * ⚠️ **원인이 둘이고 서로 다르다. 섞지 마라.** 각각을 따로 교환 대조했다(재생성 *전에*).
 *
 * ### ① 10건 — `pilotSteer` 에 **④ 사선 확보** 신설 (`src/sim/autopilot.ts`)
 ...
 * 같은 형태 · 같은 사유).
 *
 * ⭐ **건수가 진단의 증거다.** 갈린 것은 행성 0·1·3·5 의 10건이고 **행성 2·4 는 3/3 전부
 * 동일**, 행성 1 도 seed 1 하나만 갈렸다. 새 분기는 «최근접 표적과의 선분을 벽이 실제로
 * 막았을 때»만 발화하므로, 3,000틱 안에 그 상황을 한 번도 안 밟은 런은 프레임이 바이트
 * 동일하다 — 그것이 이 변경의 정확한 서명이다. 여기서 18/18 이 갈렸다면 게이트가 새는
 * 것이므로 오히려 진단이 틀린 것이다(18/18 은 입력층 전면 변경의 서명이다 — 위 절들 참조).
 *
 * ⭐⭐ **교환 대조.** `openFireLane` 첫 줄을 `return undefined` 로 눌러 돌리자 아래 ② 원인만
 * 남아 **4건**으로 줄었고, ②까지 함께 되돌리자 **18/18 전부 옛 값 그대로 그린**이었다(발산 0).
 *
