...
describe('레이싱 — 정규경로 통합(배선 실도달)', () => {
  ...
  it('(b) 채널 벽은 청크 컬링에도 소멸하지 않는다(마커 배선 실도달)', () => {
    ...
    // 흘러 컬링되지 않으므로 **전멸 가속이 덜 걸리고** 창 전진이 느려진다(실측 600틱에
    // scrollX 7556 < 8000). 이 테스트가 지키는 계약은 "코스 벽이 컬링에 살아남는가" 지
    // "몇 틱에 얼마나 나아가는가" 가 아니므로 예산만 늘린다.
    for (let i = 0; i < 900; i++) stepRacing(w);
