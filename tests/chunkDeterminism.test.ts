...
describe('chunk placement determinism (plan E, AC3)', () => {
   ...
   */
  it('지형 해저드가 전체 기믹의 2% 수준이다 (10% → 4% → 2%)', () => {
    ...
    // 해저드 2% — 표본 오차를 감안해 1~3% 구간. **상한 3 이 핵심이다**: 구값 4% 는 이 구간
    // 밖이라 회귀를 잡는다(옛 밴드 2~7 은 4% 와 2% 를 둘 다 통과시켜 아무것도 못 박고 있었다).
    expect(share('hazard')).toBeGreaterThan(1);
    expect(share('hazard')).toBeLessThan(3);
    // 마지막 2%p 는 벽이 아니라 **자석 방출기**로 갔다 — 벽은 프리팹이라 비율을 올리면 기믹
    // 예산을 넘겨 청크가 통째로 밀리고, 그러면 경로 독립성(위 다이제스트 테스트)이 깨진다.
    // 그 앞 7%p 는 벽으로 갔다(피해 없는 순수 지형). 파괴체 비율은 두 번 다 불변이라
    // 성장·보상 곡선이 이 변경에 끌려가지 않는다.
