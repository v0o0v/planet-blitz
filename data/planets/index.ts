...
export interface LootLike {
  ...
  /**
   * 엘리트 유래 표식(ADR-0038). 미지정 = 보스 확정 드랍.
   *
   * ⚠️ **설계도 축은 2026-08-08 부터 이 필드를 읽지 않는다.** 존재 이유였던 역수 보정
   * ("엘리트 유래만 ×(1/m)")이 런 단위 게이트로 대체돼 사라졌다. 필드를 남겨 둔 것은
   * `LootRecord` 쪽 표식(`src/sim/world.ts`)이 여전히 스탬프되고 다른 소비처가 생길 수 있기
   * 때문이며, **여기서는 지금 미사용**임을 명시한다 — "읽는 코드가 있겠지"로 읽히면 안 된다.
   */
  readonly elite?: 1;
}
 ...
 *
 * ## ⚠️ `planetMult` 인자를 **없앴다** (ADR-0038 역수 보정 철거)
 ...
 * 이제는 기대 획득량이 아니라 **엘리트 대 보스의 추첨 비중만** 근거 없이 틀어놓는다.
 *
 * ADR-0038 이 지키려던 불변("특산 설계도 기대 획득률은 인기 배율 밖")은 이제 보정이 아니라
 * **시그니처가** 지킨다 — 배율을 받을 자리가 없으므로 통계로 증명할 것이 아니라 **구조적으로
 * 참**이다. 인자를 `void` 로 남겨 두었다가 리뷰에서 잡혔다: 그 상태로는 배율 불변 테스트가
 * 전부 항진(어떤 값을 줘도 같은 결과)이 되어 초록이 아무것도 보장하지 않는다.
 * 대조: `tests/planetPopularity.test.ts` 가 이제 `length === 2` 로 시그니처를 직접 잰다.
 */
