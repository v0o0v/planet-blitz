 ...
 * 존재 자체를 모른 채 출시를 맞는다. 사용자 지시로 **"5런에 한 번"(20%)** 으로 올린다.
 * 유형별 가중치(`ENCOUNTERS`)는 손대지 않았으므로 조우가 떴을 때의 분포는 종전과 같다.
 *
 * ⚠️ 이 값은 `rollEncounter` 가 `worldRng.fork('encounter')` 한 **전용 스트림**에서
 * `r.chance()` 로 굴린다 — 확률을 바꿔도 소비량은 1회로 고정이라 부모 스트림은 안 밀린다.
 * 다만 조우가 **뜨는 런 자체가 10배로 늘어**나 조우 이후의 월드 해시는 당연히 갈린다.
 */
