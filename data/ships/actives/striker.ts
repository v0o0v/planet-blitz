 ...
 *
 * ## 시그니처 조작 축 — **없다**
 * ⚠️ **ADR-0049 로 전제가 바뀌었다.** 구 문장은 "스트라이커는 `signatureBit: -1` 이라 건드릴
 * 런타임 상태가 없다" 였는데, 이제 정조준 사이클(비트 24)이 `aux0` 를 볼리 카운터로 쓴다.
 * 이 파일의 6종은 여전히 aux 를 안 건드리지만, 그것은 **선택**이지 구조적 필연이 아니다.
