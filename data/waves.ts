...
//
// ## 2026-08-08 출시 전 밸런스 — 11·21 앵커 **2배**(4 → 8 · 22 → 44)
//
// 사용자 3런 실측(스트라이커·카르곤·표준 빌드 점프, `tainted:false`)이 곡선이 플레이어
// 성장을 못 따라간다는 것을 그대로 보여 줬다:
//
//   | 런 | 시간 | 결과 | HP 손실 |
//   |---|---|---|---|
//   | Lv1 / 단계1  | 108초 | **사망**(보스 25% 남김) | 100% |
//   | Lv50 / 단계10 |  80초 | 승리 | **0%** (337/337 — 한 대도 안 맞았다) |
//   | Lv100 / 단계20 | 100초 | 승리 | 5.8% (260/276) |
//
// 사용자 판정: *"레벨 50과 100은 적의 hp가 좀 더 많이 늘어나야 할 것 같다."*
//
// ## 2차 개정 — 다시 2배 (8 → 16 · 44 → 88), 그리고 이것이 **확정값**
// 1차(4→8 · 22→44) 뒤 재플레이 실측:
//
//   | 런 | HP 손실 (1차 전 → 1차 후) |
//   |---|---|
//   | Lv50 / 단계10  | 0%   → **9.5%** (305/337) |
//   | Lv100 / 단계20 | 5.8% → **10.1%** (248/276) |
//
// 사용자 판정: *"아직도 좀 쉽긴해. 좀전에 난이도 올린정도만 올리고 확정하자."* → 같은 폭
// (2배)을 한 번 더. 1차가 HP 손실을 약 +9pp 움직였으므로 2차도 비슷한 폭이면 20% 대에
// 들어온다는 계산이다.
//
// ⚠️ **이 2차 값만 사용자 플레이로 검증되지 않았다.** 1차까지는 매 판 재플레이로 확인했지만
// 2차는 "확정하자" 지시와 함께 들어와 재플레이가 없다. 출시 후 텔레메트리(ADR-0051 본선)나
// 다음 플레이 세션에서 가장 먼저 다시 볼 값이 여기다.
//
// ⚠️ **저단계 기울기 부작용**: 앵커 1 이 고정이라 11 앵커를 올리면 단계 2~10 전체가 함께
// 가팔라진다(단계 2 는 1.3 → **2.5**, 원래의 약 2배). 이것은 구간선형 곡선의 성질이고,
// 만약 단계 2~5 가 과해지면 고칠 자리는 11 앵커를 되돌리는 것이 아니라 **그 사이에 앵커를
// 하나 더 놓는 것**이다(지금은 1 → 11 사이가 직선이라 중간을 따로 못 잡는다).
//
// **단계 1 앵커는 안 건드린다** — 그 지점은 사용자가 "적절히 어렵다"(사망)로 판정했고,
// 골든 해시가 걸린 기준 무대이기도 하다(아래 `stageHpMult` 의 early-return 주석). 즉 이
// 개정은 절대 난이도를 올리는 것이 아니라 **곡선을 세우는** 것이다.
//
// 배수 2 의 근거: 같은 레인에서 플레이어 공격력이 8 → 18.24(**+128%**)로 올랐고 밀도 패스의
// 짝 축이 적 HP 를 ×1/1.3 로 내렸다 — 잡몹 TTK 가 원래의 약 **2.7배 빠르다**. 2배는 그
// 격차를 대부분 되돌리되 단계 1 쪽 기울기는 과도하게 세우지 않는 폭이다.
//
// ⚠️ **HP 는 약한 레버라는 위 경고가 여전히 유효하다** — 이 값을 올리면 난이도보다 **런 길이**가
// 먼저 늘어난다(진행이 TTK 가 아니라 스폰 카덴스에 묶여 있다). 그래도 이 축을 고른 것은
// 사용자가 지목한 축이 적 HP 이기 때문이고, 실제로 "적이 오래 살아 더 많이 쏜다" 는 경로로
// 압력이 오른다. 이걸로도 쉬우면 다음 레버는 `SEGMENTS.killGoal` 또는 적 피해다.
const HP_ANCHOR_STAGE_1 = 1; // 밴드0 대표(구 정찰) — **불변**(사용자 판정 + 골든 기준 무대)
const HP_ANCHOR_STAGE_11 = 16; // 밴드1 대표(구 교전). 4 → 8 → 16 (2026-08-08)
const HP_ANCHOR_STAGE_21 = 88; // 밴드2 대표(구 섬멸). 22 → 44 → 88 (2026-08-08)
...
/**
 * **적 피해의 단계 곡선** — 얕은 단계(1~11)에 배수 {@link ENEMY_DAMAGE_STAGE_PEAK} 를 걸고
 * {@link ENEMY_DAMAGE_STAGE_TAPER_END} 에서 정확히 1 로 물러난다 (2026-08-09, §R57).
 *
 * ## 왜 «단계가 깊을수록 큰» 곡선이 아닌가 — 실측이 그 모양을 기각했다
 * 이 축을 열기 전에 PR#405(보스 공비 1.2797) 반영 후의 기준선을 다시 떴다
 * (`bench:curve` · 96시드 · 카르곤 · 표준 빌드). 곡선은 **평평하지 않고 이미 꺾여 있었다**:
 *
 * | 단계 | 1 | 5 | 8 | 11 | 12 | 14 | 15 | 16 | 20 |
 * |---|---|---|---|---|---|---|---|---|---|
 * | 클리어율 | 89.6 | 80.2 | 85.4 | 82.3 | 70.8 | 64.6 | 65.6 | 55.2 | (§아래) |
 *
 * 즉 **밴드(60~80%) 위로 벗어난 구간은 1~11 뿐**이고 12~15 는 이미 밴드 안, 16 이후는 밴드
 * **아래**다. 여기에 «깊을수록 큰» 배수를 걸면 이미 밴드 아래인 구간을 한 번 더 누른다 —
 * §R52 ① 이 적은 **이중 보정**의 정확한 형태다. 실제로 그 모양(공비 1.0847 등비, 플레이어
 * 레벨 성장 상쇄)을 재 봤고 단계 15 가 **59.4 → 31.3%** 로 무너졌다(32시드 교환 대조).
 *
 * ⇒ **깊은 단계의 난이도는 이미 보스 곡선(A5)이 지고 있다.** 이 배수는 그 곡선이 아직 거의
 * 기여하지 않는 얕은 구간만 메우고, 보스 곡선이 인계받는 지점에서 **물러난다.** 그래서 모양이
 * 「1~11 평평 → 12·13 에서 선형 후퇴 → 14 이상 정확히 1」이다.
 *
 * ## 왜 HP 나 `killGoal` 이 아니라 **피해**인가 — 셋을 교환 대조했다 (32시드)
 *
 * | 손잡이 | 단계1 | 단계5 | 단계11 | 단계15 | 단계20 | 런당 XP(단계11) |
 * |---|---|---|---|---|---|---|
 * | 기준선 | 90.6 | 84.4 | 81.3 | 59.4 | 93.8 | 21,494 |
 * | `killGoal` 합계 ×1.48 | 78.1 | 65.6 | 71.9 | 53.1 | 87.5 | **29,238 (+36%)** |
 * | 적 피해 ×2(평평) | 78.1 | 84.4 | 65.6 | — | — | 19,943 (−7%) |
 * | 적 피해 ×1.386(단계5)·×2.25(단계11) | 90.6 | 84.4 | 62.5 | 31.3 | 31.3 | — |
 *
 * · **`killGoal` 은 듣지만 경제를 함께 끈다** — 런 길이가 69.9 → 95.5초가 되고 런당 XP 가
 *   36% 뛴다. 그러면 `EXPECTED_CARDS_PER_RUN` 과 `xpToNext` 를 **둘 다 다시 재야 한다**
 *   (`SEGMENTS` 주석). §R50 복구 레인이 방금 밴드 안에 넣은 두 지표가 다시 나간다.
 * · **적 피해는 경제 중립이다** — 처치 속도를 안 건드리므로 런 길이·XP·카드 수가 거의 그대로다.
 *   난이도만 움직이는 유일한 손잡이라 이 축을 골랐다.
 * · **적 HP 는 여전히 약한 레버다**(×1.9 에 약 3pp — 위 `HP_ANCHOR_*` 주석의 실측).
 *
 * ## ⚠️ 이 축은 **임계가 있다** — 1.4 배에서는 아무 일도 안 일어난다
 * 단계 5 에 ×1.386 을 걸었을 때 클리어율이 **0pp** 움직였고, ×2 에서 비로소 움직였다.
 * 무입력 파일럿은 카이팅해서 원래 잘 안 맞기 때문이다(§R52 ③ⓐ). **작은 계수로 미세 조정하려
 * 하지 마라** — 이 손잡이는 계단이다.
 *
 * ## ⚠️ 침공에는 걸지 않는다 (누출 차단은 소비 지점이 아니라 {@link enemyDamageScale} 이 한다)
 * 침공은 `config.stage` 가 1 이므로 «단계 1 은 1» 규약으로는 못 막는다 — 이 곡선은 단계 1 이
 * **2.0** 이다. 그래서 게이트를 단계가 아니라 **`invasion3` 유무**에 건다. `summonEnemy`
 * (침공 드론·편대·보스 무리)가 같은 상수를 소비하므로, 이 게이트가 없으면 침공 밴드가
 * 통째로 움직인다 — §R52 ② 와 정확히 같은 «거는 것을 멈추는 자리» 결함이다.
 */
export const ENEMY_DAMAGE_STAGE_PEAK = 2;
/** 이 단계까지 {@link ENEMY_DAMAGE_STAGE_PEAK} 를 그대로 건다. */
export const ENEMY_DAMAGE_STAGE_PEAK_UNTIL = 11;
/** 이 단계부터 배수가 정확히 1 이다(= 이 단계 이상은 **바이트 불변**). */
export const ENEMY_DAMAGE_STAGE_TAPER_END = 14;

/**
 * 이 단계의 적 피해 배수. 단계 12·13 은 선형 후퇴(1.667 · 1.333), 14 이상은 정확히 1.
 * ⚠️ 소비는 {@link enemyDamageScale}(`src/sim/enemyScale.ts`)를 통해서만 한다 — 침공 게이트가
 * 거기 있다.
 */
export function stageDamageMult(stage: number): number {
  const s = stage < 1 ? 1 : stage;
  if (s <= ENEMY_DAMAGE_STAGE_PEAK_UNTIL) return ENEMY_DAMAGE_STAGE_PEAK;
  if (s >= ENEMY_DAMAGE_STAGE_TAPER_END) return 1;
  return (
    ENEMY_DAMAGE_STAGE_PEAK +
    ((1 - ENEMY_DAMAGE_STAGE_PEAK) * (s - ENEMY_DAMAGE_STAGE_PEAK_UNTIL)) /
      (ENEMY_DAMAGE_STAGE_TAPER_END - ENEMY_DAMAGE_STAGE_PEAK_UNTIL)
  );
}
