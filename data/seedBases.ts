 ...
 *   - 중하(순번 08~14): 병목 회랑. 설비 종류가 넓어지고 기물 2기와 중급 보스가 선다.
 *   - 중위(순번 15~20): 굴곡 회랑. 편대·설비·기물이 만석에 가깝고 최난도 보스가 지킨다.
 *
 ...
 * `20260727010000_invasion_boss_ramp_order.sql`). SQL 램프를 고치면 이 문장들도 함께
 * 고쳐야 한다. 밴드별 실측 클리어율(참조봇·중급 장비·24시드, 2026-07-27)은
 * 하위 100% / 중하 100% / 중위 74% 이고 근거는 `tests/invasionBalance.test.ts` 가 재현한다.
 */
...
const RAW_SEEDS: readonly RawSeed[] = [
  ...
  { id: 'twin-emplacement', name: '이중 포좌', description: '회랑 소켓 네 자리에 레이저 격자까지 물렸다. 코어방에는 실드 발생기와 위상 감시자가 처음 등장한다 — 보호막을 먼저 걷어야 코어에 흠집이 난다.', ship: '이중 포좌기', level: 12 },
  ...
  // --- 중하(08~14): 병목 회랑(소켓 8). 설비 종류가 넓어지고 기물 2 · 중급 보스가 선다 ---
  ...
  // --- 중위(15~20): 굴곡 회랑(소켓 10). 편대·설비 만석 · 기물 3~4 · 최난도 보스 ---
  ...
  { id: 'steel-gateway', name: '강철 관문', description: '뇌우 제단과 같은 구성을 승급 최고 단계까지 올린 복합 관문. 파일런을 남겨 두면 여왕도 기물도 계속 되살아난다.', ship: '강철 관문기', level: 37 },
