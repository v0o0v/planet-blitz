 ...
 *   - 중하(순번 08~14): 병목 회랑. 설비 종류가 넓어지고 **코어방 기물이 다섯으로** 늘며 중급
 *     보스가 선다. 기물 충전량이 하위 2 → 중하 5 로 뛰는 것이 두 밴드를 가르는 축이다.
 ...
 * `20260727020000_invasion_band_restore.sql`). SQL 램프를 고치면 이 문장들도 함께
 * 고쳐야 한다. 밴드별 실측 클리어율(참조봇·중급 장비·24시드, 2026-07-27 밴드 목표 복원 후)은
 * 하위 100% / 중하 71% / 중위 32% 이고 근거는 `tests/invasionBalance.test.ts` 가 재현한다.
 */
