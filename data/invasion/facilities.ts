/**
 * 침공 L2 설비 카탈로그 — 임시 6종 (M7a · L4-facility).
 *
 ...
 *
 * 거동은 3갈래(`behavior`)뿐이고, 나머지는 전부 수치다 — M7c 풀 카탈로그(9종)는 이 표에
 * **append** 만 하면 된다(배열 인덱스 = catalogId 계약, 재배치 금지).
 *
...
/** 카탈로그 개수(M7a 임시분). M7c 에서 append 로 늘어난다. */
