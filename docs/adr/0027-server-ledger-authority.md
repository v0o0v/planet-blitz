...
profiles.save는 클라이언트 통짜 업로드(last-write-wins)라 credits·광물·기체·장비·스킬 투자를 임의로 위조할 수 있고, 서버 RPC 12곳이 그 값을 정본으로 읽어 왔다. 리플레이 검증을 아무리 조여도 이 경로가 열려 있으면 무의미하다 — 특히 위조 기체로 침공하면 리플레이가 내적으로 일관되어 전수 검증을 통과하고 래더가 오염된다(verifyInvasionCore에 문서화됐던 공격자 로드아웃 잔여 신뢰).

재화(크레딧·광물)와 기체 소유(기체·장비·스킬 투자)를 save에서 분리해 **서버 전용 원장**으로 옮긴다. 획득은 정산·드랍 RPC가, 소비는 건조·강화·구매 RPC가 원장에서 직접 처리하고, verify-invasion은 공격자 로드아웃을 원장과 대조한다. 지급은 즉시다 — 검증 후 지급은 ADR-0005가 기각한 비용 구조라 채택하지 않고, PvE 지급의 상한은 3중 캡(ADR-0026)이 담당한다. save에는 방어 배치·설정·진행 플래그 등 전투력에 권위가 없는 상태만 남는다.
