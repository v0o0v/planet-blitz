# Planet Blitz - apply 20260728000000_invasion_props_rebalance.sql to the remote project.
#
# This single migration is SELF-CONTAINED: it rewrites all 20 NPC defense layouts.
# Applying this one file alone lands the final state -- the earlier reseed migrations
# (20260727011000, 20260727020000) do NOT need to be applied first.
#
# What changed vs 20260727020000: the props ramp only.
#   before: nn<=4 ->0  nn<=7 ->2  else 5
#   after : nn<=4 ->0  nn<=7 ->2  nn<=14 ->4  else 3
# Rationale: fix/spatial-hash-large-radius-broadphase repaired a broad-phase defect that
# had been dropping contact candidates, which pushed the invasion bands BELOW their design
# targets. This rebalance puts them back (mid-low 70.83%, mid 40.28%). See
...
$name    = 'invasion_props_rebalance'
$file    = Join-Path $PSScriptRoot '..\supabase\migrations\20260728000000_invasion_props_rebalance.sql'
...
# --- verification ------------------------------------------------------------
Write-Host ""
Write-Host "--- verification: ramp landed on all 20 NPC rows ---"

# Expected per nn (mirrors the RAMP block in the migration header):
#   rarity     nn<=4 ->0  nn<=8 ->1  nn<=11 ->2  else 3
#   ascension  nn<=7 ->0  nn<=10->1  nn<=16 ->2  else 3
#   props      nn<=4 ->0  nn<=7 ->2  nn<=14 ->4  else 3   <-- the only line this migration moves
#   boss       nn<=4 ->none  nn<=10 ->2  nn<=16 ->0  else 1
$rows = Invoke-Sql @"
with s as (
  select g as nn,
...
Write-Host "[OK] all 20 rows match the expected ramp (boss / props / rarity / ascension)"
Write-Host ""
Write-Host "[DONE] invasion props rebalance is live on the remote seed bases."
Write-Host "       verify-invasion Edge Function was already redeployed (v36) from commit c3f61af."
