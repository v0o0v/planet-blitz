...
export default {
  ...
  palette: {
    ...
    upper: {
      ...
      bevelAmp: 16,
      /**
       * 채널 편차 0.10 — 햇빛에 바랜 석회암. **따뜻한 쪽이 아니라 차가운 쪽으로** 기울인 것이
       * 하네스 실측의 처방이다: 아르케 차저가 `rgb(110,76,52)` 인 어두운 갈색 덩어리라, 저채도
       * 회색으로 두면 베벨 립(가장 밝은 픽셀)이 그 몸통색과 ΔRGB 59 까지 붙었다(시드 3 에서
       * 배경 근접 2.21%). B 를 R 위로 올리면 같은 명도에서도 채널 거리가 벌어진다 — 채도를
       * 올리지 않고 분리를 얻는 유일한 축이고, 차가운 쪽은 넓은 안전 골짜기라 전경과 충돌하지도
       * 않는다. `crustL` 47 → 42 로 함께 내렸다(밝은 픽셀 수가 아니라 영역 평균이 판정이다).
       */
      rgb: [0.99, 1.02, 1.09],
      haloScale: 2.4,
      shadeAmp: 0.22,
