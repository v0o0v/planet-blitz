 ...
 *
 * ## 무엇을 재는가 — 방어 배치 난이도가 **아니다**
 * 런 풀 커브 계수를 1000 으로 올려 레벨업을 0회로 만들면 20기지 전부가 96시드 내내 0% 또는
 * 100% 로 고정된다(2026-07-27 실측). 즉 시드 간 승률 분산은 통째로 `state.powerupRng` 에서
 * 나오고, 적 스폰·드랍 추첨은 승패를 사실상 흔들지 않는다. **여기 "승률"은 참조봇의 파워업
 * 추첨 운 분포이지 기지 배치의 난이도가 아니다.** 읽을 때 쓸 수 있는 것은 ⓐ클리어 가능성이
 * 살아 있는가 ⓑ밴드 순서가 뒤집히지 않았는가 둘뿐이다.
 *
