 ...
 * 실제 구멍과 어긋나 있었고, 화면에서는 "불꽃이 구멍과 살짝 비껴 있다"로 보였다. **정렬 로직이
 * 아니라 기준점이 틀린 것**이라, 투영을 아무리 정확히 해도 안 맞는다.
 *
 * 그래서 링의 화면 좌표를 **픽셀에서 측정**했다(시안 화소 무게중심). 노즐 축이 바운딩 박스
 * 중심에서 한쪽으로 치우쳐 있어 좌우 대칭 `side` 하나로는 맞출 수 없고 `lateral`(중심 오프셋)이
 * 함께 필요하다.
 *
 * ## ⚠️ 완벽히 맞출 수는 없다 — 이 값은 **타협점**이다
 * 오차는 상수가 아니라 **요 위상에 따라 ±6px 흔들린다**. 5개 위상(t=0·3·5·9·13)에서 재고
 * 3축(`back`·`down`·`side`)을 전수 탐색해도 잔차가 어느 조합에서든 6~7px 남았다 — 기체가 돌면서
 * 발광 링의 **보이는 면이 달라져 무게중심 자체가 움직이기** 때문이라, 고정된 한 점으로는 원리적
 * 하한이다. 그러니 여기서 더 파지 마라.
 *
 * 그래서 "어느 위상에 맞출 것인가"의 선택이 된다. `lateral: -0.0033` 은 **화면을 여는 순간
 * (`show()` 가 `time = 0` 으로 시작한다)**에 맞춘 값이다(사용자 선택, 2026-08-04) — 그 지점이
 * 오차 최대(우측 11px)였고 사용자가 본 것도 그 프레임이었다. 대가로 중간 위상에서 좌로 몇 px
 * 벗어난다.
 *
 * 모델을 교체하면 **다시 재야 한다**(코드만 봐서는 알 수 없다). 절차: 코로나를 10px 점으로 줄여
 * 렌더 → 캔버스에서 시안 화소 무게중심과 점 좌표를 비교. 이때 **빌보드·점광을 꺼라** — 안 끄면
 * 배치하려는 발광이 무게중심에 섞여 자기 자신을 측정하게 된다.
 */
const ENGINE_AT = { back: 0.80, down: -0.16, side: 0.2325, lateral: -0.0033 } as const;
/**
