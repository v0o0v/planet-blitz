 ...
 * **0픽셀**이다.
 *
 * ## 6차 재파생 — 입력이 둘 다 틀려 있었다
 * 5차는 `실측 반폭 26 ÷ 상자 반치수 41.6 = 0.625` 로 파생했는데 **두 입력이 모두 틀렸다**:
 * 반폭은 실제 **38.3px**(26 은 27~47% 과소), 상자 반치수는 롤 스쿼시된 41.6 이 아니라 base
 * **48px** 이다({@link PLAYER_SPRITE_DISPLAY} 주석).
 *
 * 옳은 파생: `38.3 ÷ 48 = **0.798**`. 램프 끝이 실루엣 경계에 정확히 닿는 값이므로 안전 여유를
 * 두고 **0.78**(램프 끝 37.44px ≤ 38.3px)로 잡는다. 이 값에서 {@link SURFACE_STRIPS} 개 스트립이
 * **전부** 픽셀을 남기고, 5차보다 램프가 넓어져 안쪽 밴드(불투명 픽셀 1,084개)를 더 덮는다.
 */
const SURFACE_SPAN = 0.78;
 ...
 */
// ⚠️ 6차에 **일부러 올리지 않았다.** spec 은 가산이라 §2-4 밝기 예산을 쓴다. 6차 통과 조건은
// 감산 쪽 지표(박스 한정 mean |Δluma| ≥ 15)이므로 가산을 건드릴 필요가 없고, 건드리면 "세 레인을
// 다 더하고도 밝기가 착수 전보다 낮다"는 이 레인의 성과를 되돌린다. 강화는 감산 축에만 넣는다.
 ...
 *
 * ## 6차에 올린 이유 — 강도가 미달이었다
 * 5차 실측: 박스 한정 **mean |Δluma| 4.18**. 같은 도구로 재면 통과한 감산 컨투어가 **36.23**,
 * 손상 그을림이 **16.95** 다. 판면이 같은 항목군에서 가장 약했고 8× 나란히에서 육안 구별이
 * 안 됐다. 그리고 5차의 Σ dLuma 가 4차와 사실상 불변(−1,701 → −1,681)이었다 — **같은 광량을
 * 더 많은 픽셀에 더 얇게 펴 발랐을 뿐** 강도는 오르지 않았다.
 *
 * 여유는 실측으로 확인됐다: 알파를 1.0 으로 강제한 진단에서 선체가 **−245 광도**까지 내려갔다.
 * 감산이라 §2-4 밝기 예산 비용은 **0**(오히려 순감)이다.
 *
 * ## 6차 실측 진척 (같은 프레임 `L≈0.26`, 박스 한정 `diffbox`)
 * | | dLuma | 변화 px | maxD |
 * |---|---|---|---|
 * | 5차 | −2.84 | 120 | 26 |
 * | 6차(α 0.9 · 지수 0.5) | −6.19 | 510 | 170 |
 * | **6차 최종(α 1 · 지수 0.3)** | **−10.44** | **551** | **304** |
 * | (참조) 감산 컨투어 — 통과 항목 | −36.23 | 401 | 342 |
 *
 * 같은 프레임에서 **3.7배**이고 첨두 깊이(maxD 304)는 컨투어(342)에 근접한다. **통과 게이트는
 ...
 *
 * ## 왜 안쪽에 더 줘야 하는가 (6차 근인)
 * 알파는 `|중심|` 에 비례했는데 **곱연산 하강량은 바탕 광도에 비례**한다. 텍스처 밴드 실측:
 *
 * | 밴드 | 불투명 픽셀 | 평균 광도 |
 * |---|---|---|
 * | 내측(텍셀 0~11.5) | **1,084** | **103.5** |
 * | 외측(11.5~23) | 362 | 80.6 |
 *
 * 선형 램프는 **가장 강한 알파를 가장 어둡고 가장 좁은 곳에** 준다. 실측이 그것을 확증했다 —
 * α 0.4125 짜리 최외곽 스트립의 최대 하강이 **−18.7** 인데, 화면 전체 최대 **−27.4** 는 α 0.1375
...
/**
 * 플레이어 스프라이트의 **base 표시 치수**(px) — 마스크 텍스처 64×64 × `baseScale 1.5` = **96 × 96**.
 *
 * ## ⚠️ 6차 정정 — 5차의 83.2 는 base 가 아니라 롤 스쿼시된 런타임 값이었다
 * 5차는 이 값을 `{96, 83.2}` 로 두고 "두 축의 길이가 다르다"고 적었다. **83.2 는
 * `1.5 × (1 − |roll| × 0.18)` 로 계산되는 순간값**이다(|roll| ≈ 0.74). 라이브 adorner 실측은
 * `baseScaleX = baseScaleY = **1.5**` 이고 `onAttach` 시점 스프라이트는 **96 × 96 정사각**이다.
 *
 * 그래서 두 결함이 동시에 있었다:
 * 1. **`surfaceLateralHalf` 가 프로덕션에서 no-op 이다.** 정사각이므로 `min = max = 48` 이고,
 *    실측 `stripScale` 이 8장 전부 `[48, 48]` 이었다. 5차가 보고한 비등방 `(48, 41.6)` 은
 *    **런타임에 존재하지 않는다.** 화면을 실제로 바꾼 것은 `SURFACE_SPAN` 하나뿐이었다.
 * 2. **램프 게이트가 여전히 자기증명이었다.** 프로덕션이 넘기는 값(96×96)으로 재면 게이트가
 *    빨개지는데, 테스트가 롤 스쿼시된 83.2 를 넣어 초록을 봤다. 5차 뮤테이션이 red 였던 것도
 *    **런타임에 없는 가상 기하에서의 red** 였다.
 *
 * `surfaceLateralHalf` 는 그대로 둔다 — 정사각 자산에서 항등이지만 비정사각 기체가 들어오면
 * 옳게 동작하는 방어이고, **이 상수가 정사각이라는 사실이 그 항등성을 문서화한다.**
 */
export const PLAYER_SPRITE_DISPLAY = { width: 96, height: 96 } as const;

/**
 * 불투명 실루엣의 **횡 반폭 실측**(px) — `assets/player.png` 를 직접 훑어(alpha > 128) 얻은
 * **25.5 텍셀** × base 스케일 1.5 = **38.3px**.
 *
 * ## ⚠️ 6차 정정 — 5차의 26 은 27~47% 과소였다
 * 26px 은 17.3 텍셀에 해당한다. 방향이 보수적이라 §2-5 유출은 없었지만, `SURFACE_SPAN` 을
 * "실측 파생의 보수적 하한"이라고 세운 근거가 그 위에 있었으므로 **파생 자체가 무효**였다.
 *
 * {@link surfaceStripInsidePx} 의 판정 기준이며 스트립 배치와 **독립적인 입력**이다 — 배치에서
 * 파생하면 다시 자기증명이 된다. 기체 텍스처가 바뀌면 이 값도 다시 재야 한다.
 */
export const PLAYER_OPAQUE_LATERAL_HALF = 38.3;
...
export function surfaceStripAlpha(i: number, light: number, specOn: boolean): SurfaceStripAlpha {
  ...
  return {
    ...
    // 지수 < 1 — 안쪽 스트립이 상대적으로 더 강해진다. 근거는 SURFACE_SHADE_POWER 주석.
    shade: s < 0 ? SURFACE_SHADE_ALPHA * Math.pow(-s, SURFACE_SHADE_POWER) : 0,
  };
}
