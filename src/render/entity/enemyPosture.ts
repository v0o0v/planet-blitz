 ...
 *
 * 700 → 400 (3차). 2차의 듀티 측정이 **`createWorld` 직후 첫 120틱**을 잰 탓에 5.57% 로
 * 보였는데, 워밍업 300틱 뒤 실전투 구간에서 다시 재면 시드에 따라 **25~53%** 였다. 원거리
 * 커밋(피할 시간이 남은 구간)이 듀티의 대부분이라 사거리를 조이는 것이 정보 가치를 가장 덜
 * 깎는다. 플레이어 이동 속도 기준 400u 는 약 1초 거리 — 회피 입력이 실제로 필요한 창이다.
 */
export const COMMIT_RANGE = 400;
