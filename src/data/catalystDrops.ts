...
const CATALYST_DROP_AXIS: readonly {
   ...
   * 그 수가 개별 상한에 닿는 지점.
   *
   * ⚠️ **도출값과 실측값이 섞여 있다.** `id 21`·`id 38` 은 코드가 이미 정한 상한의 재기술이라
   * 튜닝 대상이 아니다(각 항목 주석 참조). `id 33`·`id 45` 는 그런 상한이 없어 **봇 런
   * 분포에서 실측**했다 — 근거는 해당 항목 주석에 있다.
   */
  ...
  },
  {
    ...
    id: 33,
    count: (c) => c.earned,
    // 포화 = **90**(2026-08-08 실측). 종전 20 은 아무도 재 본 적 없는 수였고, 실측 60런의
    // **최솟값이 37** 이라 20 은 *"어떤 런에서도 즉시 포화"* 였다 — 조건축이 아예 없는 것과 같다.
    //
    // 실측: 베르단(planet 1) × 내구 파일럿(`playerHp=1e8`, `tests/catalyst.test.ts` 의 `DURABLE`
    // 관용구) × 표준 빌드 × 20시드 × Lv10/40/70 = 60런. `earned` 분포:
    //   Lv10  p25 61 · p50 75 · p75 82 · p90 86 · max 170
    //   Lv40  p25 62 · p50 71 · p75 76 · p90 91 · max 397
    //   Lv70  p25 60 · p50 72 · p75 77 · p90 95 · max 179
    //   풀링(60런) p50 72 · p75 77 · p90 95
    // 즉사 창은 15초 주기 × 런 길이의 함수라 **레벨에 거의 불변**이다(세 레벨의 p50 이 71~75).
    // p75(77)를 잡으면 중앙값 런의 진행도가 이미 0.94 라 축이 사실상 상시 포화이므로, 레벨별
    // p90(86·91·95)의 중앙을 반올림한 **90** 을 쓴다 — 중앙값 런 0.8, 상위 10% 만 완전 포화다.
    saturation: 90,
  },
  {
  ...
  },
  {
    ...
    id: 45,
    count: (c) => c.earned,
    // 포화 = **55**(2026-08-08 실측). 종전 15 는 근거 없는 수였고 풀링 분포의 **p17** 에 불과해
    // 다섯 런 중 넷이 포화했다.
    //
    // 실측: 크라스(planet 5) × 내구 파일럿 × 표준 빌드 × 20시드 × Lv10/40/70 = 60런. `earned`:
    //   Lv10  p25 11 · p50 33 · p75 50 · p90 56 · max 65
    //   Lv40  p25 16 · p50 36 · p75 48 · p90 56 · max 62
    //   Lv70  p25 24 · p50 36 · p75 45 · p90 54 · max 73
    //   풀링(60런) p50 36 · p75 49 · p90 56
    // 하위 사분위만 레벨을 탄다(11→16→24 — 화력이 낮으면 세 배 단단한 블록을 못 뚫는다). **상위
    // 꼬리는 레벨 불변**(p90 56·56·54)인데, 부술 블록 수 자체가 코스 배치의 상한이기 때문이다.
    // 레벨별 p90 의 중앙(56)을 반올림한 **55** 를 쓴다 — 중앙값 런 0.65, 상위 10% 가 완전 포화다.
    saturation: 55,
  },
];
