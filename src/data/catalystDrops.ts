 ...
 *
 * ## 밸런스
 * 드랍률·개수 상수(`BASE_DROP_CHANCE_CP`·`MAX_DROP_CHANCE_CP`)는 전부 보수적 placeholder 다
 * (`// BALANCE`). 구조만 정본이고 계수는 출시 전 일괄 튜닝한다(defer-balance-tuning). 지금 밸런싱 금지.
 *
...
// --- 밸런스 placeholder (전부 // BALANCE — 출시 전 일괄 튜닝) --------------------

/** 장비 드랍 1건당 촉매 동반 base 확률(만분율). 400 = 4%. // BALANCE */
const BASE_DROP_CHANCE_CP = 400;
/** 촉매 드랍 게이트 확률 상한(만분율) — catalystDrop 배율이 아무리 커도 이 위로는 안 간다. // BALANCE */
const MAX_DROP_CHANCE_CP = 8000;
