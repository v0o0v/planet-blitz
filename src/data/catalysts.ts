 ...
 *
 * ── 밸런스 ── 모든 수치(perStack·dropWeight)는 보수적 placeholder 다(`// BALANCE`).
 * 구조·부호(방향)만 정본이고 계수는 출시 전 일괄 튜닝한다(defer-balance-tuning). 지금 밸런싱 금지.
 * ⚠️ `SLOT_CAP` 은 예외 — ADR-0052 가 3으로 확정한 **설계 정본**이라 밸런스 튜닝 대상이 아니다.
 */
 ...
 * 정산 캡 합성이 현행 서버 구조 안에 들어온다(4장이면 합산 최악 ×3.94 로 기준선을 넘겨
 * `CAP_HOURLY_CREDITS` 재도출이 따라온다).
 *
