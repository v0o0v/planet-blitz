/**
 * 촉매 **카르곤 특산**(id 30~32) — 카드 본체가 들어갈 자리.
 *
 * ## 왜 그룹마다 파일을 가르는가
...
/** {@link import('../catalystHooks.js').onPlayerDamagedCatalyst} 의 kargon 몫. **미배선**(위 §주석). */
export function kargonOnPlayerDamaged(state: WorldState, player: Entity, dmg: number, lethalSurvived: boolean, sources: DamageSourceMask): void {
  void state;
  void player;
  void dmg;
  void lethalSurvived;
...
export function kargonOnBulletExpired(state: WorldState, bullet: Entity, reason: BulletExpiryReason): void {
  void state;
  void bullet;
  void reason;
}
...
export function kargonOnEnemyDamaged(state: WorldState, target: Entity, dmg: number, source: Entity | undefined): void {
  void state;
  void target;
  void dmg;
  void source;
...
export function kargonOnPowerupPicked(state: WorldState, poolIndex: number, offeredIndex: number): void {
  void state;
  void poolIndex;
  void offeredIndex;
}
...
export function kargonOnResourceGranted(state: WorldState, amount: number, x: number, y: number): void {
  void state;
  void amount;
  void x;
  void y;
...
/** {@link import('../catalystHooks.js').onLootRollCatalyst} 의 kargon 몫. **미배선**(위 §주석). */
export function kargonOnLootRoll(state: WorldState, x: number, y: number, elite: boolean): CatalystLootRoll {
  void state;
  void x;
  void y;
  void elite;
...
/** {@link import('../catalystHooks.js').onWaveAdvancedCatalyst} 의 kargon 몫. **미배선**(위 §주석). */
export function kargonOnWaveAdvanced(state: WorldState, prevSegment: number, nextSegment: number): void {
  void state;
  void prevSegment;
  void nextSegment;
}
...
/** {@link import('../catalystHooks.js').onDestructibleDestroyedCatalyst} 의 kargon 몫. **미배선**(위 §주석). */
...
/** {@link import('../catalystHooks.js').stepCatalystHazards} 의 kargon 몫. **미배선**(위 §주석). */
