 ...
 *
 * 상환이 안 도는 것은 실측으로 확정이다 — 32런 합계 상환액이 부채 총액의 **0.6%** 다
 * (Lv10 53/9,040 · Lv40 59/9,600). 40 을 런 자원 규모(1~2)로 내리면 **매번 전액 상환돼
 * 대가가 사라지고**, ADR-0029 가 기각한 *"위험 없는 버프 = 세금"* 이 된다. ⚠️ 그러니 이
 * 값을 내리려면 `state.resources` 의 발행 규모를 **먼저** 바꿔야 한다.
 *
 * ## ⛔ 정정 (2026-08-08 재측정) — 대가의 상한은 이 숫자가 아니라 **그 런의 전리품 수**다
 * 이 주석의 초판이 *"빚 카드 1장이면 미상환 38 → `ceil(38/40)` = 압류 1점"* 이라 적었는데
 * 그것은 **1장 전제**의 산술이고 실제 런은 그렇지 않다. `--pick=2` 손잡이(아래)로 실제로
 * 눌러 보니 레벨업이 런당 **6~7회**라 부채가 **p50 280**(최대 800)까지 쌓인다. 산식대로면
 * `ceil(278/40) = 7점` 을 압류해야 하는데 **압류 실적은 런당 평균 0.28~0.34점**이었다 —
 * `min(전리품 수, …)`(`src/save/settlement.ts`)의 **앞항**이 물기 때문이다. 즉 초과분은
 * **통째로 증발한다.**
 * ⚠️ 다만 그 앞항이 작은 것이 봇 탓인지 설계 탓인지는 **아직 안 갈렸다** — 이 측정의 수거
 * 전리품이 p50 = 0(mean 0.22~0.41)인데 저단계는 드랍원이 엘리트 단독이라 원래 낮다.
 * **사람 런에서 이 카드의 실제 대가가 얼마인지는 여전히 미측정이다.**
 *
 * ## 재는 법 — 기본 파일럿으로는 **영영 0 이 나온다**
 * 두 파일럿이 공유하는 `pilotFreezeFrame()` 이 언제나 **오퍼 0번**을 고르는데 빚 칸은
 * `MERCANTILE_DEBT_INDEX = 2` 다. 그래서 종전 계측에서 부채가 **60런 전부 정확히 0** 이었고,
 * 그 0 은 카드가 죽었다는 뜻이 아니라 계측기가 그 칸을 안 누른다는 뜻이었다. 이제
 * `bench/runCurve.ts --pick=2`(정책 정본 `applyPilotPickPolicy`, `src/sim/autopilot.ts`)로
 * 누를 수 있다 — 그 손잡이로 32/32 런에서 부채가 실제로 섰다.
 */
