 ...
 *    **S3 가 그중 ME5 의 자리를 열었다**: 앵커 ㉔(`onCushionSettleDue`, 정산액 확정 **직전**).
 *    자리만 났고 **case 는 아직 없다** — ME5 는 여전히 미배선이다(넣는 것은 배선 레인의 몫).
 ...
 *    → **S3 가 그 앵커를 뚫었다**(㉔ `onCushionSettleDue`). ME5 는 이제 ⑳ 이 아니라 **㉔** 으로
 *    온다. ME8·ME9 는 **㉔ 으로도 못 온다** — 탕감률·임계가 `shipSignature.ts` 순수 함수
