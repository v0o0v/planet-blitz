 ...
 *  · **M9 충각 기동**(배치7 F2a 가 앵커를 열었다 — 정정): 술어가 **무적프레임 중**인데, 종전
 *    문면은 "그때는 `resolveCollisions` 가 접촉을 통째로 건너뛰어 앵커가 아예 안 불린다"였고
 *    그 줄 번호(`world.ts:4308`)도 낡았었다 — **틀렸다.** 실제로는 `invulnerable` 게이트
 *    (`world.ts` ~4558)가 `if (invulnerable) return;` 으로 조기 반환하는 그 자리가 **앵커
 *    자체**다 — 반환 직전이라 접촉 상대가 아직 스코프에 살아 있다. 그 자리를 연 것이
 *    `onContactInvuln(state, player, target)`(`skillHooks.ts` 신설, 접촉형 넷만 호출)이다.
 *    아직 `case` 가 없어 무연산 — M9 본체는 여전히 이 파일(배선 레인) 몫이다.
