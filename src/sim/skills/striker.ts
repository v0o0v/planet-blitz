 ...
 *
 * ## ⚠️ F5·F10 은 여기 **없다** — 앵커 ⑯ 으로도 닿지 않는다
 *  · **F5 조준선 관통**: 술어가 *"자동 조준 표적이 `player.angle` 기준 반각 20° 콘 안인가"* 인데
 *    이 레코드에는 표적까지의 **거리**(`targetDist`)만 있고 **방위**가 없다. 훅이 최근접 적을
 *    다시 고르면 `nearestTarget` 선택 규칙의 두 번째 사본이 생기고(그 함수는 `world.ts` 소유라
 *    leaf 가 부를 수도 없다), 그것이 정확히 `targetDist` 를 레코드에 실은 이유다. 열려면
 *    `targetAngle`(또는 판정 결과 불린) 한 칸을 `world.ts` 가 실어야 한다 — **앵커 확장이라
 *    이 레인의 몫이 아니다.**
