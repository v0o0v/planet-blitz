...
/**
 * **침공 런에는 런 내 레벨업이 없다** — 항상 {@link NO_LEVELUP}(0)을 돌려준다.
 *
 * ## 결정 (2026-08-10, 사용자 지시 "침공에서는 레벨업 카드 없앤다")
 * 침공은 엔드게임 PvP 배치전이다. 기준선도 「만렙 기체가 기본 수비대를 어느 정도 클리어하는가」로
 * 잡혀 있다 — 즉 **런 안에서 강해지는 축이 아니라, 이미 강해진 것을 시험하는 무대**다. 그래서
 * 런 풀 파워업 3택을 통째로 걷어냈다. 침공 결과는 이제 장비·계보·배치·조작으로만 결정된다.
 *
 * ⚠️ **이 결정이 뒤집는 전제가 하나 있다.** `bench/invasionBands.ts` 머리말은 "시드 간 승률
 * 분산이 통째로 `state.powerupRng` 에서 나온다"고 적어 두었고, 그래서 그 벤치의 "승률"은 배치
 * 난이도가 아니라 파워업 추첨 운의 분포라고 스스로 경고했다. 추첨이 사라졌으므로 그 경고는
 * 이제 해당 없고, **같은 이유로 그 벤치의 승률이 비로소 배치 난이도를 가리키게 된다.**
 * 반대로 과거에 그 분산에 기대어 잡힌 밴드 목표선(하위 ≥85% 등)은 근거가 사라졌다 —
 * 기준선을 사용자가 다시 잡는 중이므로 이번 레인에서는 손대지 않는다.
 *
 * ## 메타 성장은 그대로다 (없앤 것은 '카드'뿐)
 * 경험치 이원화(ADR-0036)라 젬은 두 풀에 각각 들어간다:
 *   · 런 풀 `state.xp`      — 3택 파워업. **침공에서 소비되지 않는다**(여기가 0이므로).
 *   · 메타 풀 `state.xpTotal` — 기체 영구 레벨. **침공에서도 그대로 쌓인다.**
 * 즉 침공을 돌면 계정은 계속 성장하고, 그 성장이 다음 침공의 시작 전투력이 된다. 두 풀이
 * 애초에 갈려 있어서 이 결정이 한 줄로 끝났다.
 *
 * ## 여기서 0 을 돌려주는 이유 — 분기점을 하나로 유지한다
 * 별도 boolean 이나 `WorldConfig` 필드를 만들지 않는다. 이 파일은 이미 **술어를 두 곳에 적어**
 * 대가를 치른 적이 있다(2026-07-27: 판정부만 침공 커브로 갈고 HUD 분모는 `xpToNext` 로 남아
 * 침공에서 분모가 11배 부풀어 표시됐다. 레벨 0 구간만 두 커브가 우연히 같아 발견이 늦었다).
 * 그래서 분기는 {@link xpToNextForRun} 하나뿐이고, 소비처는 전부 `need > 0` 를 본다.
 *
 * ## 폐기된 것 (이력 — 다시 만들지 마라)
 * 이 함수는 곡선이었다: `10 + 6L`(2026-07-27) → `10 + 66L`(2026-08-10 밀도 레인).
 * 두 번째 재기준화는 밀도 축이 L1 총 스폰을 25 → 240 으로 올리며 "침공은 젬 획득량이 훨씬
 * 적다"는 전제를 깼기 때문이었다(런당 종료 레벨 3.2 → 20.8 → 6.6). **그 계수가 처치량에
 * 종속된다는 사실 자체는 PvE `xpToNext` 에 그대로 남아 있다** — 거기서 같은 사고가 먼저
 * 있었다(`killGoal` 80→240 에 `10+13L` 이 남아 레벨업 14~18회). 침공 쪽 곡선을 되살릴 일이
 * 생기면 그 종속을 먼저 재라.
 *
 * ⚠️ **`verify-invasion` EF 재배포는 필요하다.** 골든 바이트 불변은 재배포 불필요의 근거가
 * 못 된다(루트 `README.md` `## 서버 배포`) — 번들 소스가 바뀌었으면 번들도 바꾼다.
 */
...
/**
 * `xpToNextForRun` 이 "이 런에는 레벨업이 없다"를 알리는 값. 0 이다.
 *
 * 별도 boolean 을 두지 않은 이유는 **분기점을 하나로 유지**하기 위해서다(아래 `xpToNextForRun`
 * 주석의 사고 — 술어를 두 곳에 적었더니 HUD 분모와 판정 임계가 갈렸다). 소비처는 이미
 * `need > 0` 을 확인하는 형태거나(HUD `xpPct`) 여기서 함께 고쳤다(`checkLevelUp`).
 */
export const NO_LEVELUP = 0;
...
function checkLevelUp(state: WorldState): void {
  ...
  // 런 풀 커브는 런 구조에 따라 갈린다. 분기는 `xpToNextForRun` 하나에만 있고 HUD 도 같은
  // 함수를 쓴다(그 주석의 분모 불일치 결함 — 술어를 두 곳에 적었던 대가).
  const need = xpToNextForRun(state);
  // `need <= 0` = **이 런에는 레벨업이 없다**(침공, 2026-08-10 사용자 지시). 0 을 그냥
  // 흘리면 `state.xp >= 0` 이 항상 참이라 매 틱 레벨업이 터진다 — 반드시 여기서 끊는다.
  if (need <= NO_LEVELUP) return;
