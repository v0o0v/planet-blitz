...
// 배치7 F2b — 발사부 leaf(`emitVolley`, W2b 선결). `world.ts` 는 여전히 `actives.js` 를 통해
// 간접 의존해 왔지만(→ `activeTypes.js`), `autoAttack` 이 볼리를 직접 그 leaf 로 넘기려면
// 여기서도 명시적으로 불러야 한다. `activeTypes.ts` 는 `WorldState` 를 **타입으로만** 당기므로
// (그 파일 헤더 근거) 이 edge 는 순환이 아니다 — 타입 import 는 런타임에 완전히 지워진다.
