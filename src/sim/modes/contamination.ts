...
export const CONTAMINATION_NODE_HP_BASE = 40000;

/*
 * ## 62,000 → 40,000 의 근거 (2026-08-02 난이도 복구 2차, 2,940~4,550런)
 *
 * 패배 런의 **정화율이 0.11** 이었다 — 노드 10개 중 1개를 깨고 61초에 죽는다(승리 런은 1.00 ·
 * 80.6초). 즉 오염 무대의 사인은 확산 압박도 오염 지형 피해도 아니라 **목표 총 HP 를 생존 창
 * 안에 감당하지 못하는 것**이었다. 그래서 유입 1.6배도 `CONTAMINATION_CELL_DAMAGE` 6→0 도
 * 무반응이었다(둘 다 구속 조건이 아니었다). 진단 지표는 `purifyEnd`(밸런스 하네스, 관측 전용).
 *
 * | 지표 | 62,000 | 40,000 |
 * |---|---|---|
 * | 클리어율 | 14.1% | **24.5%** |
 * | 보스도달 | 88.2s | **74.9s** |
 *
 * ⚠️ **이 상수는 난이도 레버인 동시에 페이싱 레버다.** 목표 게이트형 무대에서 "보스 도달
 * 90초"와 "클리어율 60~80%"는 같은 하나의 레버에 매달려 있어 **둘 다 만족시킬 수 없다.**
 * 그래서 생존 축을 따로 만들었다 — {@link CONTAMINATION_DAMAGE_SCALE}. 이 무대의 최종 조합은
 * **HP 40,000 + 기울기 0.06 + 피격 배율 0.26** 이고 그 결과가 25.1% → **61.7%**(밴드 안) ·
 * 보스도달 77.1 → 84.4s 다. 세 값은 **함께** 정해진 것이라 하나만 되돌리면 안 된다.
 */
/**
 ...
 * 타면 고레벨에서 과하게 무거워진다.
 */
export const CONTAMINATION_NODE_HP_SLOPE = 0.06;
 ...
 * (추격 Lv100 타임아웃 13.3% → 30%대). 목표 총 HP 가 단계로 오르는 속도가 플레이어 화력 성장을
 * 앞지르면, 안 죽는 런이 승리로 가는 게 아니라 **시간 초과로 간다**. 기울기를 낮춰 그 격차를
 * 좁혔다 — 무대의 절대 난이도는 `_HP_BASE` 가, 단계 축 기울기는 이 계수가 진다는 분업은 그대로다.
 *
