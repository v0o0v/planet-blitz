...
/**
 * 뒤 경계 압박 틱당 피해(iframes 간격 적용, 즉사 아님).
 *
 * ## 8 → 16 (2026-08-03) — 이 무대의 유일한 난이도 축이다
 * 봇 수정(§R1) 이후 아르케는 **93.9%** 로 밴드(60~80%) 위 최상단에 고정돼 있었다. 패배 런의
 * 피해 귀속을 보면 지형 0.26 · 접촉 0.29 로 **어느 압박도 지배적이지 않았다** — 그냥 쉬웠다.
 *
 * ⚠️ **압박 축은 전부 절벽이다. 마진(`_MARGIN`)을 건드리지 마라.**
 *
 * | 조합 | 클리어율 |
 * |---|---|
 * | 마진 120 · 피해 8(전) | 93.9% |
 * | 마진 200 · 피해 8 | 92.9% (거의 무반응) |
 * | 마진 420 · 피해 8 | **14.2%** ← 절벽 |
 * | 마진 120 · **피해 16** | **64.9%** |
 * | 마진 120 · 피해 34 | **14.7%** ← 절벽 |
 *
 * 마진은 200 까지 아무 일도 없다가 420 에서 무대가 붕괴한다(압박 구간이 넓어지면 봇이 상시
 * 그 안에 들어가 지속 피해로 죽는다 — 이분화). 피해량 쪽이 그나마 연속적이지만 이쪽도
 * 8 → 16 에서 29pp, 16 → 34 에서 50pp 움직인다. **한 번에 2배 이상 움직이지 마라.**
 *
 * 16 에서 보스 도달 87.3s 로 페이싱(90초)도 지킨다. TODO(밸런스).
 */
export const RACING_PRESSURE_DAMAGE = 16;
