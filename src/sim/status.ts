...
// --- 정지(스트라이커 S9 선결) -------------------------------------------------
//
// ⚠️ 여기에 `STASIS_DURATION` 같은 **기본 지속 상수를 다시 만들지 마라.** 배치7 F1 이 그것을
//    자리표시자(90)로 한 번 세웠는데 **아무도 쓰지 않았다** — S9 배선 레인이 설계 수치
//    (`45 + 5×Lv`)를 자기 함수 안에서 직접 계산했고, 그게 옳다. 지속이 레벨의 함수라
//    스칼라 상수로 표현할 수 있는 값이 애초에 아니다.
//
//    죽은 export 로 남겨 두면 이 저장소의 지배적 실패 모드를 하나 더 만든다 — "미리 열어 둔
//    자리"가 *"배선이 있다"* 는 착각을 만드는 형태다(`skillHooks.ts` 앵커 ⑮ 주석이 같은
//    경고를 남겼다). 그래서 지웠다. 밸런스 수치의 정본은 `skills/striker.ts` 의 S9 본체다.
