 ...
 * leaf 에 둔다(`skills/mallowStatus.ts` · 아크캐스터 레인의 `skills/arccasterChain.ts`).
 * `catalystHooks.ts`·`filmBurst.ts` 가 같은 사상으로 앵커를 자기 파일에 들고 있는 선례다.
 *
 * ## ⚠️ 머지 주의 — 이 파일을 **두 레인이 각자 신설했다**
 * `lane/arccaster-anchors` 가 `onChainParams`(전격 연쇄 파라미터, `applyChain` 진입)를 담은
 * 같은 이름의 파일을 먼저 세웠고, 이 레인(말로우)이 `onEnemyStatusExpired` 를 담은 같은 이름의
 * 파일을 세웠다. 둘은 **목적이 같고 내용이 겹치지 않는다** — 머지는 **합집합**이다(헤더는 이
 * 쪽이 두 경우를 다 적고 있으므로 이쪽을 남기고, `onChainParams` 와 그 `ChainParams` 인터페이스
 * 및 `skills/arccasterChain.js` import 를 아래에 그대로 얹으면 된다).
 *
 * ## 공통 계약(다른 앵커와 동일)
 *  - 첫 줄은 `if (!state.skillsOn) return;` — 미투자 런은 **바이트 불변**.
 *  - 다음은 `switch (state.sigBit)` 기체 게이트. `break;` 를 반드시 붙여라.
 *  - RNG 를 소비하지 마라.
 *  - ⚠️ **훅에서 엔티티를 스폰하지 마라.** 호출부가 `state.entities` 순회 안이다.
 */

import type { WorldState } from './world.js';
import type { Entity } from './entities.js';
import { SIG_MALLOW_CUSHION } from './shipSignature.js';
