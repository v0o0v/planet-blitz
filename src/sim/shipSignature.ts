 ...
 * 미사용 상위 비트(18~21)** 를 재활용한다. 신규 LoadoutConfig 필드도 신규 hashWorld 폴드도
...
/** 비온 — 처치 시 포자 적립, 임계 도달 시 자동 소환. */
export const SIG_BION_SPORE = 21;

/**
 * 시그니처 비트 전량(선언 순서 = 타입 id 1~4 순서). 테스트·검증이 하드코딩 목록 대신
...
// --- ④ 비온: 포자 -------------------------------------------------------------
/** 첫 소환에 필요한 처치 수. */
export const SPORE_BASE_KILLS = 12;
/** 스케일 구간을 하나 넘길 때마다 늘어나는 요구 처치 수. */
export const SPORE_STEP_KILLS = 4;
/** 요구치가 한 단계 오르는 누적 처치 간격. */
export const SPORE_SCALE_KILLS = 60;
/** 요구 처치 수 상한(후반에 소환이 사실상 멈추지 않도록 고정). */
export const SPORE_MAX_KILLS = 40;

/**
 * 런 누적 처치 `kills` 시점에서 **다음 포자 1기 소환에 필요한 처치 수**(정수).
 *
 * 배선 계약(world.ts 레인용): 소환 이후 처치 카운터를 0으로 리셋하고, 그 카운터가
 * `sporeThreshold(누적 처치)` 이상이 되는 틱에 소환한다. 누적이 늘수록 요구치가
 * 계단식으로 올라 후반 소환 폭주를 막는다. 단조 비감소이며 상한에서 평평해진다.
 */
export function sporeThreshold(kills: number): number {
  const k = Math.trunc(kills);
  if (k <= 0) return SPORE_BASE_KILLS;
  const need = SPORE_BASE_KILLS + Math.floor(k / SPORE_SCALE_KILLS) * SPORE_STEP_KILLS;
  if (need >= SPORE_MAX_KILLS) return SPORE_MAX_KILLS;
  return need;
}
