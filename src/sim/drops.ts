 ...
 * (§R51 `PILOT_LEVEL_GROWTH` 도입 후 재측정 — 42 를 그대로 뒀을 때의 장비 드랍이 마지막 열):
 *
 * | 단계 | Lv | 초 | 카드 | 42 일 때 장비 드랍 | 32 일 때 장비 드랍 |
 * |---|---|---|---|---|---|
 * | 1  | 5   | 70.4  | 30.32 | 1.82 | **2.20** |
 * | 5  | 25  | 72.4  | 30.63 | 2.07 | **2.36** |
 * | 11 | 55  | 80.0  | 32.81 | 2.11 | **2.17** |
 * | 20 | 100 | 110.5 | 44.08 | 2.32 | **2.66** |
 *
 * 얕은 세 점 평균 31.3 → **32**. 단계 20 만 44 로 튀는데, 그 점은 레벨 배율이 붙은 만렙이
 * 오래 살아남아 런이 길기 때문이고 옛 표의 단계 21(43.52)과도 일치한다 — 한 상수로 접으면
 * 만렙이 조금 더 받는다(위 표 2.66, 밴드 2~3 안).
 *
