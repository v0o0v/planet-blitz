...
export const enum PhantomCarry {
  ...
  executionerStacks = 1,
  /**
   * DI10「공허 계약」의 **대가(최대 HP −8%)를 이미 물렸는가**. 0 = 아직 안 물림 · 1 = 물림
   * (값 규약 1 — 0 이 자연 센티넬).
   *
   * `Stage` 가 아니라 `Carry` 인 이유가 이 칸의 존재 이유 그 자체다: 설계 문면이 대가를
   * *"레벨 무관 고정 −8%"* 로 못박아 **런당 정확히 1회**여야 하는데, `Stage` 는 구간마다
   * 리셋되므로 의뢰 구간을 넘길 때마다 대가가 다시 물려 **HP 가 계단식으로 녹는다**.
   *
   * ⚠️ 같은 축의 **DI8 은 진입마다 가산이라 규율이 정반대다**(`phantomEntry.ts` 의
   * `player.maxHp += …`). DI8 을 선례로 베끼면 이 칸이 무의미해진다 — 두 스킬은 `maxHp` 라는
   * 같은 필드를 만지지만 **적용 횟수 규약이 다르다.**
   *
   * ⚠️ `state.tick` 으로 "런 시작인가"를 판정하려던 시도가 배치7 배선 레인에서 한 번 있었는데,
   * `tick` 은 **구간마다 0 으로 리셋**돼 그 술어로는 1회를 보장할 수 없다. 그래서 칸이 필요하다.
   */
  voidCovenantApplied = 2,
}
