 ...
 *
 * ## ⚠️ 2026-08-08(2차) — 이 값은 이제 **base** 다
 * 촉매 **드랍 축** 실측 배율이 이 값을 스케일한다(사용자 지시: *"설계도와 의뢰서도 아이템이다"*).
 * 실효 = `scaleGateChanceCp(3000, mult)` 이고 최대 배율 ×3.0 에서 **90%** 다.
 * 위 "런 클리어시 30%"는 **드랍 축 미발동 런**(= 무촉매 런)에서만 참이다.
 *
 * SQL 미러: `ISSUE_CHANCE_CP` (`issue_commission_for_run` 4b 단계,
 * `20260808090000_drop_axis_scales_issue_and_caps.sql`).
 */
export const COMMISSION_ISSUE_CHANCE_CP = 3000;

/**
 * 클라가 주장하는 **드랍 축 배율의 형식 상한**(centi, 100 = ×1.00). 300 = ×3.00.
 *
 * ## 값의 출처는 손 계산이 아니라 전수 스윕이다
 * `scripts/catalystCapSweep.ts` 가 48C3 = 17,296 조합(특산 필터 후 12,430 유효)을 **공명 포함**
 * 으로 전수 돌려 드랍 축 최댓값 **×3.0000** 을 냈다(#15 extraction + #20 resonance +
 * #34 berdan-royal-jelly, 공명 harvest:weak snare). 촉매 3장분만 세는 `axisCapMult` 는
 * ×2.9 를 내놓으므로 **그것을 쓰면 정직한 최대 조합이 잘린다** — 드랍축 공명이 7종이다.
 *
 * ## 서버는 이 값을 **클램프한다, 거부하지 않는다**
 * 범위 밖 주장은 잘라서 진행하고, 키가 없는 구 클라는 100 으로 접혀 **정확히 종전 30%** 를
 * 받는다(하위 호환이 산술로 보장된다). 거부하면 캐시된 구 클라가 조용히 막힌다.
 *
 * SQL 미러: `MAX_LOOT_MULT_CENTI` (`issue_commission_for_run` 4b 단계).
 */
export const COMMISSION_MAX_LOOT_MULT_CENTI = 300;

/**
 * 프로필당 **하루 발령 상한**(발령 건수 총량, 시도 수가 아니다).
 *
 * ## 왜 하루 축인가 — 시간 축은 넣을 수 없다
 * 드랍 축 배율은 클라 주장이고 서버가 검증할 수 없다. 그런데 **공격자의 거짓말이 배율
 * 하나뿐**이라, 최상위 정직 유저는 같은 배율을 정당하게 주장한다:
 *
 *   위조 천장(시간) = 20 주장/h × min(1, 0.30 × 3.0) = 18/h
 *   정직 천장(시간) = 20 주장/h × min(1, 0.30 × 3.0) = 18/h   ← 같은 수다
 *
 * 둘을 가르는 신호가 없으므로 18 미만의 시간 캡은 공격자를 조이는 만큼 **정확히 같은 크기로
 * 정직한 플레이어를 조인다**. 18 이상은 무연산이다. 그리고 ADR-0026 의 기준(치터 이득이
 * **최상위 정직 유저**의 파밍 속도로 유계)은 두 수가 같으므로 비율 **1.0** 으로 이미 충족돼
 * 있다 — 배율 도입 전(6 대 6)과 같은 비율이다.
 *
 * 실제로 비어 있던 것은 **하루 축**이다(배율과 무관하게 원래 없었고, 배율이 그 구멍을 3배로
 * 키웠다):
 *
 *   정직 천장(하루) = 16시간 × 20 주장/h × 0.9 = 288  (Binomial(320, 0.9) sd 5.4 → 360 은 +13sd)
 *   위조 천장(하루) = 24시간 × 18/h = 432  →  **360 으로 닫힌다**
 *
 * ⚠️ **자인**: 시간 축의 여유(18/h)는 남는다. 더 조이려면 배율을 검증 가능하게 만들어야 하고
 * (= 서버가 촉매 주입 목록에서 재계산), 그 길은 조건을 무시한 **무조건 배율**이 되어 헌장
 * §상한 근거 규율과 충돌한다. **별도 결정으로 남긴다.**
 *
 * SQL 미러: `CAP_COMMISSIONS_PER_DAY` (`issue_commission_for_run` 4c 단계).
 */
export const CAP_COMMISSIONS_PER_DAY = 360;
