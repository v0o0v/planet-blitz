...
/**
 * 계급별 구간 수 — **전 계급 2**(2026-08-03 밴드 복구 레인에서 2/3/4/5 에서 내렸다).
 *
 * ## ⚠️ 계급이 더 이상 구간 수로 갈리지 않는다 — 그 이유가 실측에 있다
 * 밸런스 큐 §R7 사용자 판정이 지목한 축이 이것이고, 96시드 실측이 그 축의 **모양**을 확정했다.
 * 구간마다 HP 가 승계되고(회복이 없다) 성장이 없는 고정 화력 파일럿이 도므로, 클리어율은
 * 구간 수에 대해 기하급수보다 가파르게 떨어진다:
 *
 ...
 *
 * (굵은 값이 2/3/4/5 시절의 실측이다.) 목표 밴드가 **전 계급 80% 이상**이므로 3구간 이상은
 * 어떤 조합으로도 들어오지 못한다 — 그래서 전 계급을 2 로 내렸다. 계급 차이는 이제
 * **단계 분포**(`[1, grade]`)와 보상 배율이 진다.
 *
 ...
 * 읽는 곳: Phase D(정예 소집령 스포너).
 *
 * ## 2026-08-03 — 150 → 25 (정예 전용 난이도 축)
 * 근거·대조표는 {@link COMMISSION_ELITE_OVERLAP_MAX} 주석에 함께 있다(둘은 **한 쌍으로**
 * 정해졌다 — 상한만 올리면 그 상한까지 차오르는 데 걸리는 시간이 그대로라 압박이 안 붙는다).
 */
export const COMMISSION_ELITE_OVERLAP_DELAY_TICKS = 25;

/**
 * 의뢰 **구간 하나당 보스 전 일반 웨이브 세그먼트 수** — `WorldConfig.maxSegments` 로 실린다.
 * **2026-08-03 밴드 복구 레인에서 3 → 2.**
 *
 ...
 *
 * ## 왜 3 → 2 인가 — **구간당** 축이 필요했다 (96시드 실측)
 * `COMMISSION_SEGMENT_COUNT` 를 2 로 내려도 계급3 이 78.5% 로 밴드(80%) 아래에 남았다.
 * 구간 수는 "몇 번 도는가"만 정하고 "한 번이 얼마나 무거운가"는 이 값이 정한다 —
 * 3 은 `SEGMENTS[0..2]`(killGoal 10+40+46 = **96**)를, 2 는 `SEGMENTS[0..1]`(10+40 = **50**)을
 * 소화해야 보스가 나온다는 뜻이다. 2026-08-01 페이싱 조정(§P5)이 무대 하나를 90초로 늘린 것이
 * 의뢰에 구간 수만큼 곱해져 실린 것을 되받는 자리다.
 *
 * | 구간 수 2 · 니플헤임 제외 클리어율 | 계급1 | 계급2 | 계급3 | 계급4 |
 * |---|---|---|---|---|
 * | 웨이브 세그먼트 3(전) | 81.0% | 74.4% | 73.8% | 76.1% |
 * | **2** | 87.3% | 87.2% | **78.5%** | 86.6% |
 * | **2 + 단계 분포 `[1, grade]`** | **90.5%** | **87.2%** | **84.6%** | **83.6%** |
 *
 * ## 그리고 이것이 **틱 상한 초과 1건을 닫은 축**이다
 * main(074ec88)에서 계급4 typical 시드 373 의 1구간(톡사르/단계3)이 **10,421틱**으로
 * {@link COMMISSION_SEGMENT_TICK_CAP}(9,000)을 넘겨 있었다 — 넘으면 밸런스가 아니라 **서버
 * 거부**다. 변종 실측(시드 373·379, 구간 1개):
 *
 * | 변종 | 373/계급4(톡사르 st3) | 379/계급3(톡사르 st1) |
 * |---|---|---|
 * | main 기준 | **10,421**(패) | **9,131**(패) |
 * | 봇 전리품 수거 끔(`LOOT_SEEK_RADIUS`=0) | 10,421(패) — **완전 동일** | 9,131(패) |
 * | 저단계 완화 끔(`OBJECTIVE_LOW_STAGE_RELIEF`=1) | 8,501(패) | 6,801(패) |
 * | 조준 가중치 끔 | 6,801(패) | 9,131(패) |
 * | **이 값 3 → 2** | **6,076(승)** | **6,923(승)** |
 *
 * ⚠️ **원인은 전리품 수거가 아니었다** — 반경을 0 으로 해도 틱이 한 틱도 안 움직인다. 초과한
 * 두 구간은 **둘 다 톡사르(오염)**이고, 직전 PvE 레인의 생존 축(§R13 조준 가중치 · §R16 저단계
 * 완화)이 "일찍 죽던 런"을 "살아남아 갈리는 런"으로 바꾼 것이 근인이다. 그 둘은 PvE 지표를
 * 지고 있으므로 되돌리지 않고, **의뢰 전용인 이 값**으로 구간을 짧게 만들어 닫았다.
 *
 * ⚠️ 정예 소집령은 이 값을 쓰지 않는다({@link COMMISSION_ELITE_WAVE_SEGMENTS} = 1) — 그 오버라이드가
 * 사문이 되지 않도록 이 값은 **1 이 되면 안 된다**(`tests/verifyCommission.test.ts` 가 못 박는다).
 *
 * 읽는 곳: Phase D(`buildRunConfig` 의뢰 경로) · Phase G(구간 길이 계측) ·
 * `supabase/functions/verify-commission/verifyCommissionCore.ts`(**EF 재배포 필요**).
 */
export const COMMISSION_WAVE_SEGMENTS_PER_SEGMENT = 2;

/**
 * 정예 겹침의 **절대 상한** — 압박이 누적돼도 동시 정예가 이 수를 넘지 않는다.
 * **2026-08-03 밴드 복구 레인에서 6 → 32**(짝인 {@link COMMISSION_ELITE_OVERLAP_DELAY_TICKS}
 * 150 → 25 와 함께).
 *
 * ## 왜 정예에 **별도 축**이 필요했는가 (ADR-0043 방향 계약 복원)
 * 정예 소집령은 계급2 좌표로 재므로 {@link COMMISSION_SEGMENT_COUNT} 를 내리면 **함께 쉬워진다** —
 * 계급 축과 정확히 반대 방향이 필요하다. main 실측 88.6% 는 밴드(40~85%) 위였고 chain 계급2
 * (57.1%)를 역전해 "정예가 chain 보다 어렵다"는 ADR-0043 방향 계약이 뒤집혀 있었다.
 *
 * **96시드 실측 — 정예 전용 축 4종 중 셋이 사실상 무반응이었다**(구간 수 2 · 니플헤임 제외):
 *
 * | 정예 축 | 클리어율 | 비고 |
 * |---|---|---|
 * | 기준(MAX 6 · DELAY 150) | 97.4% | 밴드 위 |
 * | `OVERLAP_MIN` 2 → 3 | **94.9% (기준과 바이트 동일)** | ⚠️ 아래 참조 |
 * | `REGROUP` 240 → 90 | 96.2% (**오히려 쉬워진다**) | 빨리 보충 = 킬 공급 증가 |
 * | `ELITE_WAVE_SEGMENTS` 1 → 2 | 83.3% | ❌ **틱 상한 초과 33건**(최대 14,028) — 기각 |
 * | `MAX` 6 → 12 단독 | 94.9% (**무반응**) | 차오르는 속도가 그대로다 |
 * | MAX 12 · DELAY 60 | 84.6% | 천장 85 에 0.4pp — 너무 얇다 |
 * | **MAX 32 · DELAY 25** | **65.4%** | 채택. p99 4,856 · max 5,142(상한의 57%) |
 *
 * ⚠️ **`OVERLAP_MIN` 은 난이도 손잡이가 아니다 — 실측이 그것을 증명했다.**
 * `decideEliteDeploy` 는 `alive >= cap` 이면 상한을 한 칸 올리고 **그 틱에 그냥 투입**하므로,
 * 시작 상한이 2든 3이든 투입 판정이 같다. 2→3 이 96시드에서 **한 틱도 안 바뀌었다**.
 * 실제로 미는 것은 (절대 상한, 차오르는 속도) **쌍**이다.
 *
 * ⚠️ 32 는 "화면에 항상 32기"가 아니다. 상한은 **직전 정예를 못 치우는 동안에만** 오르고
 * 전멸하면 {@link COMMISSION_ELITE_OVERLAP_MIN} 으로 되돌아간다 — 빨리 치우는 플레이어는
 * 이 값을 영영 보지 않는다(ADR-0043 의 압박 누적을 그대로 쓴다).
 *
 ...
 */
export const COMMISSION_ELITE_OVERLAP_MAX = 32;
