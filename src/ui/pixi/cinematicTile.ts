...
const HOVER_LERP = 9;
/**
 * 미세 부유 진폭(카드 높이 기준)과 주기(초).
 *
 * ## ⚠️ 진폭이 0 인 이유 — "테두리가 번쩍거린다"(사용자 신고 2026-08-04)
 * 예전 값 0.004(카드 높이 352 → **1.4px**)는 "아주 작게"라는 의도였지만, 문제는 크기가
 * 아니라 **정수 픽셀이 아니라는 것**이었다. 카드에서 가장 밝은 요소는 위/왼쪽 1px 베벨 립
 * ({@link BEVEL_LIGHT})이고, 그 한 줄이 매 프레임 서브픽셀로 오르내리면 GPU 가 프레임마다
 * 다른 비율로 리샘플한다 — 눈에는 **테두리가 깜빡이는 것**으로 보인다. 게다가 타일마다 위상이
 * 달라({@link phaseOf}) 8칸이 제각각 번쩍여, 정지 화면인데도 격자 전체가 지글거렸다.
 *
 * 반올림으로 무마할 수 없다. `Math.round(bob)` 은 1.4px 진폭에서 0↔1 을 오가는 **계단**이
 * 되어 오히려 더 눈에 띈다. 부유 자체를 0 으로 두는 것이 정답이다 — 이 화면의 생동감은 호버
 * 리프트·후광·Ken Burns 줌이 이미 충분히 만든다(전부 사용자 조작에 반응하는 것들이다).
 * 상수만 0 으로 두고 코드는 남긴다: 훗날 카드를 정수 격자에 스냅하는 부유를 넣는다면 여기다.
 */
const FLOAT_RATIO = 0;
...
/**
 * 제목·설명색. 위계를 크기와 **색·굵기 양쪽**으로 벌린다.
 *
 * ## 부제는 밴드를 어둡게 해서가 아니라 **글자를 올려서** 푼다 (4차 판정 MED-처방5)
 * 옛 `#9a8f7d` 는 실측 대비 **4.26:1** 로 WCAG AA(4.5) 미달이었다. 이 라운드가 만든 결함이
 * 아니라(v5 4.17 · v2 6.96) 밴드 램프 작업이 **고치지 못하고 지나친** 것이다.
 *
 * ⚠️ 고치는 방향이 중요하다. 밴드 하단을 더 어둡게 하면 대비는 오르지만 **램프 8장 동일성**
 * (4차에서 검증 통과: 상단 46.1–46.6 / 하단 36.7–40.0)이 깨진다. 배경색은 그대로 두고
 * 전경만 올린다 — 대비는 두 항의 비인데, 이번엔 건드려도 되는 항이 하나뿐이다.
 *
 * `#b8ac97`(L601 173.2) 검산 — 밴드 램프 전 구간에서 목표 5.5:1 을 넘는다:
 *  - 밴드 상단 rgb(62,45,32) = 가장 밝은 바탕 = **최악의 경우** → 4.12 → **5.86**
 *  - 실측 지점 rgb(50,37,27) → 4.66 → **6.63**
 *  - 밴드 하단 rgb(46,33,24) → 4.90 → **6.97**
 * 제목과의 위계도 유지된다(L601 230.7 vs 173.2, 굵기 700 vs 400).
 */
const TITLE_FILL = 0xf0e6d2;
const DESC_FILL = 0xb8ac97;
...
export function makeCinematicTile(o: CinematicTileOpts): CinematicTile {
  ...
  // --- 호버 후광(카드 뒤, 가산) -------------------------------------------------
  // ⚠️ **이 후광은 카드 밖으로 나가는 유일한 밝은 요소이고, 정지 상태에서는 알파가 정확히 0
  // 이다**(`glow.alpha = 0.34 * hover`). 4차 판정 "카드 아래로만 +50L 발광 번짐(결함B)"의
  // 용의자로 지목됐으나 원인이 아니었다. 판별 근거 두 가지:
  //  ① 구조 — 카드 밖으로 나가는 것은 이 후광(정지 시 α0)과 **검은** 그림자 2단뿐이다.
  //     `orb`·`spec`·`warm` 은 전부 `clip` 안이라 카드 사각으로 마스크된다. 어둡게는 해도
  //     밝게 할 수 있는 요소가 정지 화면엔 없다.
  //  ② 실측(base_v6.png) — **카드가 한 번도 없던 좌측 여백(x18~50)** 이 같은 y 구간에서
  //     L 88.9 → 112.3 으로 **+23.4 상승**한다. 카드 아래(+31.3)와 같은 방향·같은 크기이고,
  //     절대값은 오히려 여백 쪽이 훨씬 밝다(112 vs 69). y570~580 가로 스캔에서도 밝기가
  //     카드 주기를 따르지 않는다(카드1 발치 안에서 L17~112 로 요동, 최고점은 카드 밖 x1700).
  // → 배경 원화의 밝은 바닥 대역이다(Lane A 소관). 여기서 그림자를 더 키우지 않는다.
