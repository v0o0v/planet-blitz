/**
 * Research lab overlay (DOM — M3 Phase D2, plan §4, AC1/AC7 · ADR-0049 flat 재편).
 *
 * The base's 연구소: visualises the three skill axes(화력·생존·기동, 축당 10개 flat 스킬),
 * lets the pilot spend banked skill points into a skill(`investSkill`), respec the whole
 * tree for a credit cost(`respecSkills`/`respecCost`). Synergy·capstone·티어 선행 조건은
 * ADR-0049 가 전부 폐기했다 — 축당 10스킬은 처음부터 전부 투자 가능하고, 효과는 sim 안의
 * 메커닉이라 이 화면이 미리 계산해 보여줄 파생 수치가 없다(`node.desc` 한 문장이 전부다).
 *
...
#pb-lab .pb-grid { display:grid; grid-template-columns:repeat(2,1fr); gap:7px; }
...
export class ResearchLab {
  ...
  /** 현재 편집 대상 = 활성 기체의 타입 정의(축 수·스킬 수의 단일 출처). */
  ...
  private render(): void {
    ...
    // 축 수·구성은 활성 기체 타입이 정한다(3축 고정 가정 제거).
  ...
  private treePanel(treeIndex: number): HTMLElement {
    ...
    sub.textContent = `${t('lab.bar.invest')} ${axisInvested(this.invest(), def, treeIndex)}pt`;
    panel.appendChild(sub);

    // flat 구조(ADR-0049): 축당 스킬은 티어 없이 전부 동시에 투자 가능하다. 한 그리드에
    // 그 축의 스킬을 전부 늘어놓는다 — 구 구현의 "티어별 행 묶기"는 티어 개념과 함께 사라졌다.
    const grid = document.createElement('div');
    grid.className = 'pb-grid';
    const { start, end } = shipTreeRange(def, treeIndex);
    for (let index = start; index < end; index++) {
      if (nodes[index] === undefined) continue;
      grid.appendChild(this.nodeEl(index, accent));
    }
