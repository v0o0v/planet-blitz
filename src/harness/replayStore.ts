 ...
 * 한 줄을 보여 주는" 조작을 하기 위한 **순수 변환**만 담당한다. 재생 자체는 기존 관전 경로
 * (`src/ui/replaySpectate.ts` + main.ts 의 `beginSpectate`)가 하고, 이 모듈은 그 경로에 넣을
 * 값을 만들고 검증한다.
 *
 * ## shape 검증을 재구현하지 않는 이유
 * 재생 가능 여부의 정본은 `isPlayableReplay`(replaySpectate.ts:28)다 — seed 가 유한수이고
 * inputs 가 비어 있지 않은 배열인지. 하네스가 별도 술어를 두면 "하네스에선 되는데 관전에선
