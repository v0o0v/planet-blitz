...
import { CI_SEEDS, FIXED_SEEDS } from './balance/seeds.js';
 ...
 *
 * ⚠️ **정본은 `src/bench/balance/seeds.ts` 로 옮겼다** — 예산제 밸런스 하네스가 같은 목록을
 * 써야 "같은 시드로 쟀다"는 주장이 성립하기 때문이다. 값은 바이트 동일이라 이 파일의 기존
 * 산출물(M8 매트릭스 · 곡선 스윕)은 그대로 재현된다.
 *
