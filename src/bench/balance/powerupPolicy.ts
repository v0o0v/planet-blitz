 ...
 *
 * ## ⚠️ 기준값이 작으면 **정수 반올림이 변이를 삼킨다** (2026-08-03 실측)
 * 처음에는 실제 기본값(발사 간격 6틱)을 그대로 썼는데, `fp-cadence`(발사 간격 −8%)가
 * `round(6 × 0.92) = 6` 이라 **아무것도 안 바뀌어** `'unknown'` 으로 분류됐다. 이것은 분류기의
 * 결함이 아니라 `powerups.ts` 헤더가 경고하는 그 함정(정수 틱)이 실물로 드러난 것이다 —
 * 기본 발칸에서 이 파워업은 진짜로 무연산이다.
 *
 * 분류는 "이 파워업이 **어느 축을 건드리려 하는가**"를 알아야 하므로, 스텁 기준값은 배율 변화가
 * 반올림에 삼켜지지 않을 만큼 **크게** 잡는다. 무기별 기본값에서 실제로 먹히는지는 이 축이 답할
 * 질문이 아니다(그건 `tests/powerupMagnitude.test.ts` 와 실측 런의 몫이다).
 *
 * 함정에 걸린 실물 3종과 닫힌 형식(`c(1 − m) < 0.5`)은 밸런스 큐 **§R22** 에 등재돼 있다 —
 * `fp-cadence`(벌컨 6틱 · 빔 4틱에서 처음부터 무동작) · `rapid-fire`(벌컨에서 1회만) ·
 * `dash-coils`/`sv-evasion`(대시 하한 12에서). **고치지 않았다**(처방이 밸런스 변경이다).
 */
