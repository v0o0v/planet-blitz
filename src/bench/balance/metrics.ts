...
export const RUN_METRICS: Readonly<Record<string, MetricDef>> = {
  ...
  /**
   * 런당 **수거한** 장비 수. `state.loot` 는 수거분만 센다(바닥 잔존은 `lootGround`).
   *
   * ## ⚠️ `winMean` 인 이유 — 전 런 평균은 이 밴드와 정의가 어긋난다 (2026-08-03)
   * 밴드의 정본은 ADR-0035 §3.1 이고 그 셈은 **완주 런** 기준이다(보스 확정 1 + 엘리트 기대
   * 1.5). `tests/drops.test.ts` 의 "런당 장비 유입 통합 가드"도 완주 런으로 잰다. 그런데 이
   * 지표가 `mean`(전 런)이던 동안에는 패배 런이 보스 드랍 없이 섞여 들어와, **승률이 낮아질수록
   * 장비유입 게이트가 빨개졌다** — 드랍 축을 하나도 건드리지 않아도 그렇다.
   *
   * 그래서 이 게이트는 드랍 설계가 아니라 승률을 재고 있었다. 실제로 두 정의는 **동시에 만족될
   * 수 없다**: 전 런 평균을 2.0 위로 올리려면 드랍 목표를 1.5 → 2.6 으로 올려야 하는데, 그러면
   * 완주 런이 4.38 개가 되어 `drops.test.ts` 가 빨개진다(실측). 정본 쪽에 맞춘다.
   *
   * 수거 실패분은 이 값에서 여전히 빠진다 — 그건 의도다(`lootGround` 가 그 축을 따로 본다).
   */
  lootPerRun: {
    label: '장비유입',
    kind: 'winMean',
    ...
    target: {
      min: 2,
      max: 3,
      source: 'ADR-0035 §3.1 — 완주 런당 장비 2~3개(보스 확정 1 + 엘리트 기대 1.5)',
      scope: 'overall',
    },
  },
  /**
